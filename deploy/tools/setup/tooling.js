...
function exec(command, args, cwd, sshControlPath) {
  ...
  var stderr_output = [];
  function combineStderr(data) {
    stderr_output.push(data);
  }

  if (!(_.isString(command) && _.isString(cwd))) {
    deferred.reject(
      new Error('Both command and working directory must be given, not ' + command + ' and ' + cwd)
    );
  } else if (
    !_.every(args, function(a) {
      return _.isBoolean(a) || _.isString(a) || _.isNumber(a);
    })
  ) {
    ...
    proc.on('close', function(code) {
      ...
      if (code !== 0) {
        spec_error.message = util.format(
          'command `%s` exited with error code %d (in %s)\n%s',
          command_with_args,
          code,
          cwd,
          stderr_output.join('').replace(/^/gm, '\t| ')
        );
  ...
  return Q.nfcall(fs.stat, clone_path)
    .then(
      function(/*stats*/) {
        ...
        return exec(cmd, pull_args, clone_path).catch(function(git_pull_err) {
          var friendly_error = new Error(
            util.format(
              'Error doing a `git pull --ff-only for repo "%s": %s',
              repo,
              git_pull_err.message
            )
          );

          friendly_error.friendly_headline = util.format(
            'Repo "%s": could not do a fast-forward only pull',
            repo
          );
          friendly_error.friendly_explanation = [
            ...
            'you should perform more complicated merges by hand.)'
          ].join(' ');
...
function startSshControlMaster(socketPath) {
  ...
  return (
    Q.nfcall(fs.stat, defaultSshControlPath)
      ...
      .catch(function(err) {
        ...
        var proc = child_process.spawn(
          ...
          [
            ...
            'ControlPersist=30s',
            '-o',
            util.format('ControlPath=%s', socketPath),
            // Enable compression
            '-o',
            'Compression=yes',
            '-o',
...
function topoSort(node) {
  ...
  var sort = _.reduce(
    ...
    function(sort, d) {
      ...
      _.each(innerSort, function(i) {
        // Because _.union does not work with objects
        if (
          !_.find(sort, function(i2) {
...
function distinctExternals(externals, checkVersionMismatch) {
  var mismatchs = {};
  var distinctExternals = _.reduce(
    externals,
    function(acc, dep) {
      var modifier = dep.version[0];
      if (modifier !== '~' && isNaN(modifier) && dep.version.indexOf('/') === -1) {
        console.warn(
          'In %s, dependency %s is not set to bugfixes only (~): %s',
          dep.source,
          dep.name,
          dep.version
        );
      }
      function sameDep(dep1, dep2) {
          ...
          .filter(function filterUnlinkedDependencies(dependency) {
            try {
              ...
              if (fs.lstatSync(depPath).isSymbolicLink()) {
                ...
                console.log(
                  '  ...linking dependency "%s" (already installed, but not as a link)',
                  dependency
                );
                return true;
              }
