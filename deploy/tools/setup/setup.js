...
var argv = require('yargs')
  ...
  .describe(
    'dry-run',
    'Do not execute link/install commands, print installation dependencies instead.'
  )
...
var errorHandler = function(err) {
  if (err.friendly_headline) {
    ...
    if (err.friendly_explanation) {
      console.error(err.friendly_explanation);
    }
  } else {
...
function cloneOne(repo) {
  return tooling.clone(repo).then(_.identity, function handleCloneRepoError(err) {
...
function cloneAll(stack, done) {
  ...
  return tooling
    ...
    .then(function() {
      var cloneReposPromises = _.map(stack, function(repo) {
        return cloneOne(repo);
      });
      return Q.all(cloneReposPromises);
    })
    .then(function getClonedDeps(repos) {
      return _.chain(repos)
        .map(function(clonedRepo) {
          return tooling.getPkgInfo(roots, clonedRepo);
        })
        .map('dependencies')
        .flatten()
        .where({ internal: true })
