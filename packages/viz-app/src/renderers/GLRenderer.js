...
export class GLRenderer extends Renderer {
  ...
  setCamera2d(left, right, bottom, top) {
    ...
    mat2d.translate(
      mvpMatrix,
      mvpMatrix,
      vec2.fromValues((left + right) * lr, (top + bottom) * bt)
    );
  ...
  renderEdges() {
    const { gl, numEdges, elementsPerPoint, programs: { edges }, buffers: { springs } } = this;
    edges.use();
    edges.bindVertexAttrib(
      springs,
      'curPos',
      elementsPerPoint,
      gl.FLOAT,
      false,
      elementsPerPoint * Float32Array.BYTES_PER_ELEMENT,
      0
    );
  ...
  renderMidEdges() {
    const {
      ...
      buffers: { midSprings, midSpringsColorCoord }
    } = this;

    if (colorTexture == null) {
      midedges.use();
      midedges.bindVertexAttrib(
        midSprings,
        'curPos',
        elementsPerPoint,
        gl.FLOAT,
      ...
      midedgestextured.bindVertexAttrib(
        midSprings,
        'curPos',
        elementsPerPoint,
        gl.FLOAT,
  ...
  renderPoints() {
    const {
      ...
      elementsPerPoint,
      programs: { points },
      buffers: { curPoints, pointSizes, pointColors }
    } = this;
    points.use();
    points.bindVertexAttrib(
      curPoints,
      'curPos',
      elementsPerPoint,
      gl.FLOAT,
    ...
    points.bindVertexAttrib(pointSizes, 'pointSize', 1, gl.UNSIGNED_BYTE, false, 0, 0);
    points.bindVertexAttrib(
      pointColors,
      'pointColor',
      4,
