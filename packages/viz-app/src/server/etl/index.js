...
const formParser = multer({ storage: multer.memoryStorage() }).fields(
...
function etlWorker(config, activeCB) {
  ...
  app.post(
    ...
    function(err, req, res, next) {
      ...
      if (res.headersSent) {
        let unseenError = new VError(
          err,
          'Unable to report ETL request error to client because a response has already been sent. The error will only be logged here.'
        );
...
function notifySlackAndSplunk({
  ...
  function makeUrl(server) {
    ...
    var url = sprintf('%s/graph/graph.html?type=%s&dataset=%s&info=true', domain, type, name);
