...
function configureWorkers(config, convict, activeCB) {
    ...
    try {
        ...
        logger.fatal(
            authMiddlewareError,
            'An error occured loading the Express.js authentication middleware. Because this could compromise security, server will now terminate.'
        );
    ...
    appRouter.post(
        '/etl',
        selectWorkerRouter('etl', configureEtlWorker, false),
        requestErrorHandler
    );
    appRouter.get(
        '/graph/graph.html',
        authenticate,
        selectWorkerRouter('viz', configureVizWorker, true),
        requestErrorHandler
    );
                    ...
                    logger.warn(
                        {
                            ...
                            err: inUseError,
                            workerRouterStartTime,
                            workerRouterLongevityMS: Date.now() - workerRouterStartTime
                        },
...
function requestErrorHandler(err, req, res, next) {
    logger.warn(
        { req, res, err },
        'An error occured while processing the HTTP request. Responding to the request with an error code and message, if possibly.'
    );
