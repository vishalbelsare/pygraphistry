...
const colorTexture = Observable
    .bindNodeCallback(fs.readFile)(imgPath)
    .do((raw) => logger.trace('Loaded raw colorTexture', raw.length))
    .mergeMap(
        (raw) => Observable.bindNodeCallback(deflate)(
            raw, {
                output: new Buffer(
                    Math.max(1024,Math.round(raw.length * 1.5))
                )
            }
        ),
...
function textureHandler(req, res) {
    ...
    colorTexture.subscribe({
        next(data) { res.set('Content-Encoding', 'gzip').send(data); },
        error(err) {
