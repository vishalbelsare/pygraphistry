...
export function authenticateMiddleware(conf) {
  ...
  if (!conf.get('authentication.passwordHash')) {
    log.info(
      `Authentication disabled because "authentication.passwordHash" is not set in your config. Clients will not be prompted for a username and password, and all requests will be allowed. Requests will have their 'username' property set to the default values of "${authorizedUsername}".`
    );
