...
function addSocketEventListeners(req, getDataSource) {
    return function addEventListeners(socket) {
        ...
        const disconnectEvents = Observable.fromEvent(socket, 'disconnect').do(() =>
...
function enrichLogsAndEmitSocketWithMetadata(socket) {
    const {
        ...
        handshake: { query: { dataset, debugId, usertag } }
    } = socket;
    ...
    logger.info(
        { req: { ...socket.handshake, ...socket.request }, remoteAddress },
        'Connection Info'
    );
