...
import { Label as LabelComponent, Labels as LabelsComponent } from 'viz-app/components/labels';
...
let Label = container({
  ...
  fragment: (fragment, { pinned, showFull } = {}) => {
    return (
      (false && !pinned && !showFull && `{ type, index, title, globalIndex }`) ||
...
let Labels = ({
  ...
  if (selection) {
    labels = labels
      .filter(({ type, index }) => type !== selection.type || index !== selection.index)
      .concat(selection);
  }
  if (highlight) {
    labels = labels.filter(
      ({ type, index }) => type !== highlight.type || index !== highlight.index
    );
