...
let Inspector = container({
  ...
  mapFragment: (fragment, props) => {
    ...
    const { rows, columns, templates, openTab, sortKey, sortOrder, searchTerm } = getInspectorState(
      fragment
    );
...
function getTemplatesForTab(templates = [], componentType) {
  ...
  return Array.from(templates)
    .filter(
      template => template && !blacklist[template.name] && template.componentType === componentType
    )
    .sort(sortTemplatesByName);
}
