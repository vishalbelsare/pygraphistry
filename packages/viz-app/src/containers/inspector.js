...
import { Inspector as InspectorComponent } from 'viz-app/components/inspector';
import {
    selectInspectorTab,
    setInspectorSortKey,
...
import { WithGridLayout, WithScrollPosition } from 'viz-app/components/data-grid';

let Inspector = container({
    renderLoading: true,
    fragment: (fragment, props = {}) => {
        const { tabs, rows, query, columns, templates, allTemplatesLength } = getInspectorState(
            fragment
        );
        ...
        return `{
            ...
            rows: {
                ${query.reduceRight(
                    ...
                    `length, [${from}...${to}]: {
                        _title, _index, ${columnNames.map(x => `'${escapeQuotes(x)}'`).join(', ')}
                    }`
                )}
            }
        }`;
    ...
    mapFragment: (fragment, props) => {
        ...
        const lastRenderedRowIndex =
            Math.max(rowsPerPage, endRow - startRow) + (scrollTop + rowHeaderHeight) / rowHeight;

        const loadingRow = { rowIsLoading: true };
        const rowsToRender = new Array(rowsPerPage + 2);
        const numPages = (rows.length && Math.ceil(bodyHeight / (rowHeight * rowsPerPage))) || 0;
        const page = Math.max(
            1,
            Math.min(numPages, Math.floor(lastRenderedRowIndex / rowsPerPage))
        );
        ...
        for (let i = -1, n = rowsPerPage + 2; ++i < n; ) {
            ...
            if (startRow + i >= rows.length) {
                ...
                rowsToRender[i] = row ? row : loadingRow;
            }
        }
        ...
        return {
            ...
            colsPerPage: Math.max(
                1 + colsPerPage,
                Math.min(colsPerPage + 2, cols.length - startCol)
            )
        };
    },
    dispatchers: {
...
Inspector = compose(WithScrollPosition, WithGridLayout, Inspector)(InspectorComponent);
...
function getInspectorState(fragment = []) {
    ...
    query = query.map(
        ({ key, val }) =>
            key === 'sortKey'
                ? `sortBy:${escapeQuotes(val)}`
                : key === 'searchTerm' ? `search:${escapeQuotes(val)}` : val
    );

    const rows = query.reduce(
        (node, key, index) => node[key] || (node[key] = index < query.length - 1 ? {} : []),
        fragment.rows || {}
    );
...
function getTemplatesForTab(templates = [], componentType) {
    if (componentType === 'event') {
        ...
        return Array.from(templates)
            .filter(
                template =>
                    template &&
                    !blacklist[template.name] &&
                    template.componentType === componentType &&
                    template.isInternal === false &&
                    template.name !== 'type'
            )
    ...
    return Array.from(templates)
        .filter(
            template =>
                template && !blacklist[template.name] && template.componentType === componentType
        )
        .sort(sortTemplatesByName);
}

function coerceSortKey(templates, sortKey) {
    return sortKey || (templates.length > 0 && templates[0] ? templates[0].name : '_title');
}
