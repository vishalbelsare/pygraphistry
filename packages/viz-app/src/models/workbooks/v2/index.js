...
export default function migrateV2(workbook, options) {
  ...
  for (const viewId in viewsById) {
    ...
    if (filters.controls && filters.controls[0]) {
      filters.controls[0].items = $ref(
        `workbooksById['${workbook.id}'].viewsById['${view.id}'].filters`
      );
    }
    if (exclusions.controls && exclusions.controls[0]) {
      exclusions.controls[0].items = $ref(
        `workbooksById['${workbook.id}'].viewsById['${view.id}'].exclusions`
      );
    }
    if (histograms.controls && histograms.controls[0]) {
      histograms.controls[0].items = $ref(
        `workbooksById['${workbook.id}'].viewsById['${view.id}'].histograms`
      );
    }
