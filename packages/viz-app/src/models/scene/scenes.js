...
for (const itemName in items) {
  ...
  while (++triggersIndex < triggersLen) {
    ...
    (triggersToNames[trigger] || (triggersToNames[trigger] = {}))[itemName] = true;
  }
}
...
function generateScene(scene, items, models, programs, textures) {
  ...
  return function generate() {
    const generatedScene = render.reduce(
      (scene, name) => {
        const {
          buffers: cBuffers,
          textures: cTextures,
        ...
        for (const bindingName in bindings) {
          ...
          if (datasource === 'VERTEX_INDEX') {
            ...
            cModes[datasource] = 2;
          } else if (datasource === VBODataSources.HOST || datasource === VBODataSources.DEVICE) {
        ...
        for (const bindingName in otherBuffers) {
          ...
          if (datasource === VBODataSources.HOST || datasource === VBODataSources.DEVICE) {
            cServerBuffers[modelName] = true;
          }
