...
export function histogram(
    { name = 'degree', yScale = 'none', dataType = 'number', componentType = 'point' } = {},
...
export function histogramFilterQuery(histogram, binIndexes = []) {
    let query;
    const { bins, binType, minValue, binWidth, identifier, componentType } = histogram;
    ...
    if (binType === 'nodata' || binIndexes.length <= 0) {
        ...
        return histogramBinQuery(histogram, {
            exclude: false,
            values: [betweenVals[0], betweenVals[betweenVals.length - 1]]
        });
    }
    ...
    const { exclude, values } = binIndexes.reduce(
        ({ exclude, values }, i, x, xs, bin = bins[i]) => ({
            ...
            values: bin.exclude ? values : [...values, ...bin.values]
        }),
...
export function histogramBinQuery(histogram, bin, shouldCoerceTypeToBinaryPredicate = false) {
    ...
    const queryProperties = {
        ...
        values: !exclude
            ? undefined
            : bins.reduce((vals, { exclude, values } = {}) => {
                  if (!exclude && values && values.length) {
                      vals.push.apply(vals, values);
                  }
                  return vals;
...
export function histogramBinHighlightQuery(histogram, bin) {
    ...
    if (!identifier) {
        logger.error({
            msg: 'histogramBinHighlightQuery without identifier',
            histogram,
            bin,
            exn: new Error()
        });
