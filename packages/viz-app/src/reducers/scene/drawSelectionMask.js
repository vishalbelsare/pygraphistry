...
export function drawSelectionMask(actions) {
  ...
  const drawnMaskSelections = SceneGestures.pan(drawMaskStarts)
    .repeat()
    .mergeMap(drag =>
      drag
...
export function toValuesAndInvalidations({ rect, falcor, refreshMask }) {
  return {
    falcor: refreshMask ? falcor : falcor.withoutDataSource(),
    values: [
    ...
    invalidations: !refreshMask ? undefined : [`inspector.rows`, `selection.histogramsById`]
  };
}
