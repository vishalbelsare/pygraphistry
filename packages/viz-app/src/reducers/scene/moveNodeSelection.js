...
export function moveNodeSelection(actions) {
    const moveNodeStarts = SceneGestures.startFromActions(
        actions
            .ofType(SELECTED_POINT_TOUCH_START)
            .filter(
                ({ event, simulating, selectionType }) =>
                    !simulating && selectionType === 'select' && !event.getModifierState('Shift')
            )
    );

    const movedNodeSelections = SceneGestures.pan(moveNodeStarts)
        .repeat()
        .mergeMap(drag =>
            drag
                .stopPropagation()
...
function callMoveSelectedNodes(point) {
    const { falcor } = point;
    return falcor.call('moveSelectedNodes', [
        {
            x: point.worldX - point.worldXOrigin,
            y: point.worldY - point.worldYOrigin
        }
