...
export function pickNodeSelection(actions) {
    const selectionStarts = SceneGestures.startFromActions(
        actions
    ...
    const highlightMoves = SceneGestures.moveFromActions(
        actions
    ...
    const selectionTaps = SceneGestures.tap(selectionStarts, { delay: tapDelay })
        .repeat()
        .mergeMap(tap => tap.map(point => (point.type = 'tap') && point));

    const movesWithHitmapUpdate = highlightMoves.combineLatest(hitmapUpdates, point => point);

    const movesWhileNotAnimating = isAnimating.switchMap(
        (animating = true) => (animating && Observable.empty()) || movesWithHitmapUpdate
    );

    const tapsWhileNotAnimating = isAnimating
        .switchMap((animating = true) => (animating && Observable.empty()) || selectionTaps)
        .startWith({ time: 0 });

    const pointsAndElements = movesWhileNotAnimating
        .auditTime(0, Scheduler.animationFrame)
        .map(point => {
            point.element = hitTestN(point.renderState, hitTestNTextures, point.x, point.y, 10);
            return point;
        })
    ...
    const distinctPointsAndElements = pointsAndElements
        ...
        .map(point => {
            ...
            element.type = dim <= 0 ? 'none' : dim === 1 ? 'point' : 'edge';
            return point;
        });
...
function filterDistinctPointsAndElements(pointA, pointB) {
    ...
    return elementA && elementB && elementA.dim === elementB.dim && elementA.idx === elementB.idx;
}
...
function toValuesAndInvalidations({ type, falcor, element, highlightEnabled }) {
    return {
        falcor,
        ...(element.type !== 'none'
            ? selectionValuesAndInvalidations(type, element, falcor.getPath(), highlightEnabled)
