...
function updateHistogram(action$) {
  return action$
    ...
    .switchMap(({ falcor, key, value }) => falcor.set($value(key, value)));
}

function removeHistogram(action$) {
  return action$.ofType(REMOVE_HISTOGRAM).mergeMap(({ id, falcor }) => falcor.call('remove', [id]));
}
...
function highlightHistogramBin(action$) {
  return action$
    ...
    .takeUntil(action$.ofType(CLEAR_HIGHLIGHT))
    .repeat()
    .switchMap(({ binID, falcor }) => falcor.call('highlightBin', [binID]));
}
...
function filterBinsOnDrag(actions) {
  ...
  const filterBinsMoves = SceneGestures.moveFromActions(actions.ofType(BIN_TOUCH_MOVE));

  const filterBinsStartsById = SceneGestures.startsByIdFromActions(actions.ofType(BIN_TOUCH_START));

  const brushPans = filterBinsStartsById.take(1).mergeMap(filterBinsStarts =>
    SceneGestures.pan(filterBinsStarts, filterBinsMoves, filterBinsEnds, filterBinsCancels)
  ...
  function scanBinIDsAndAnchor(memo, [binID, x, falcor, range_, binIsFiltered]) {
    ...
    memo[1] = binID;
    memo[0] = binID > cursor ? anchor : binID < cursor ? anchor : cursor;
    return memo;
  }
  ...
  function multicastBrushPans(drags) {
    return drags
      ...
      .merge(drags.takeLast(1).map(toRemoteFilterCall));
  }
  ...
  function toLocalFilterValues(
    [
      ...
      range_,
      binIsFiltered,
      range = _range(Math.min(anchor, cursor), Math.max(anchor, cursor) + 1)
    ]
  ) {
    return falcor.withoutDataSource().set($value('range', range), $value('filter.enabled', true));
  }

  function toRemoteFilterCall([anchor, cursor, falcor, range_, binIsFiltered]) {
    let range = [];
    ...
    return Observable.merge(
      ...
      falcor.withoutDataSource().set($value('range', range), $value('filter.enabled', true))
    );
  }
}
