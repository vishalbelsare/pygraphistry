...
export class SceneGestures extends Gestures {
    ...
    static moveFromActions(actions) {
        return SceneGestures.to(
            SceneGestures.move.bind(SceneGestures), actions);
    }
    static endFromActions(actions) {
        return SceneGestures.to(
            SceneGestures.end.bind(SceneGestures), actions);
    }
    static cancelFromActions(actions) {
        return SceneGestures.to(
            SceneGestures.cancel.bind(SceneGestures), actions);
    }
    static tapFromActions(actions) {
        return SceneGestures.to(
            SceneGestures.tap.bind(SceneGestures), actions);
    }
    static pressFromActions(actions) {
        return SceneGestures.to(
            SceneGestures.press.bind(SceneGestures), actions);
    }
    static panFromActions(actions) {
        return SceneGestures.to(
            SceneGestures.pan.bind(SceneGestures), actions);
    }
    static to(gesture, actions) {
        return new this(actions
            .multicast(createSubject, (actions) =>
    ...
    withPointIndexes() {
        return this.withLatestFrom(curPoints, (point, { buffer }) => {
            const { rect } = point;
            point.pointIndexes = !rect ? [] : pointIndexesInRect(
                new Float32Array(buffer), rect.tl, rect.br
            );
            return point;
        })
    }
    mapToWorldCoords() {
        return this.map((point) => {
            ...
            const { xOrigin, yOrigin,
                    movementXTotal, movementYTotal,
                    renderState: { camera, canvas } } = point;
    ...
    moveCameraInWorldCoords() {
        return this.map((point) => {
            ...
            if (camera) {
                ...
                const { center, width = 1, height = 1,
                        simulationWidth = 1, simulationHeight = 1 } = camera;
