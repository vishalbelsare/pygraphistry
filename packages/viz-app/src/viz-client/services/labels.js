...
export function loadLabels(loadViewsById, labelsByIndex = {}, labelOffsetsByType = {}) {
    ...
    return function loadLabelsByIndexAndType({
        ...
        .mergeMap(
            ({ workbook, view, labelType, offsets, labelIndex }) => loadLabelByIndexAndType({
                offsets, labelType, labelIndex
            }),
    ...
    return function loadLabelByIndexAndType({ offsets, labelType, labelIndex }) {
        ...
        return (labelType in labelsByType) ?
            ...
            labelsByType[labelType] = Observable.defer(() => {
                    ...
                    const upperBound = labelIndex < offsets.length ?
                        offsets[labelIndex + 1] - 1 :
                        undefined; // Upper bound will be undefined for last label

                    if (upperBound !== undefined && lowerBound >= upperBound) {
                        throw new Error('Invalid byte range indicated at', labelType, labelIndex);
                    }

                    const byteEnd = upperBound !== undefined && upperBound.toString ? upperBound.toString(10) : '';
                    const byteStart = lowerBound !== undefined && lowerBound.toString ? lowerBound.toString(10) : '';

                    // First label: start can be 0, but end must be set.
                    // Last label: start is set, end unspecified, okay.
                    if (!byteStart && !byteEnd) {
                        throw new Error('Undefined labels range request', labelType, labelIndex, byteStart, byteEnd);
                    } else if (upperBound && upperBound - lowerBound > LABEL_SIZE_LIMIT) {
                        throw new Error('Too large labels range request', labelType, labelIndex, byteStart, byteEnd);
                    }
