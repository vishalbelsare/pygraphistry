...
export function setupSocketHandlers(args) {
    ...
    const onError = Observable
        .fromEvent(socket, 'error')
        .mergeMap((reason) => Observable.throw(
            `Connection error (reason: ${reason || ''} ${reason && reason.description || ''})`
        ));

    const onDisconnect = Observable
        .fromEvent(socket, 'disconnect')
        .mergeMap((reason) => {

            $('#simulation').parent().addClass('disconnected');
    ...
    const onLoadingStatus = Observable
        .fromEvent(socket, 'update_loading_status')
        .pluck('message');
