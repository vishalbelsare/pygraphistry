...
export function loadHistograms(loadViewsById) {
    return function loadHistogramsById({
        workbookIds,
        viewIds,
        histogramIds,
        masked,
        ...
        return loadViewsById({
            ...
            .mergeMap(
                ...
                ({ workbook, view }, histogramId) => ({
                    workbook,
                    view,
                    histogram: view.histogramsById[histogramId]
                })
            )
            .concatMap(
                ...
                ({ workbook, view }, histogram) => ({
                    workbook,
                    view,
                    histogram: (view.histogramsById[histogram.id] = histogram)
                })
            );
    };
}
...
export function loadSelectionHistograms(loadViewsById) {
    ...
    return function loadSelectionHistogramsById({
        workbookIds,
        viewIds,
        histogramIds,
        refresh = true,
        ...
        return loadHistogramsById({
            ...
            ({ workbook, view, histogram }) => {
                const { nBody: { vgraphLoaded } = {}, selection: { histogramsById } = {} } = view;
                ...
                return computeSelectionMasks({
                    view,
                    emptyIfAllSelected: masked
                }).mergeMap(selectionMasks =>
                    computeHistogram({
                        view,
                        masked,
                        refresh,
            ...
            ({ workbook, view }, histogram) => ({
                workbook,
                view,
                histogram: (view.selection.histogramsById[histogram.id] = histogram)
            })
        );
    };
}

export function computeMaskForHistogramBin({
    view,
    histogram,
    bin,
    ...
    if (errors.length) {
        console.error({
            msg: '====BAD computeMaskForHistogramBin',
            errors,
...
function computeHistogram({
    ...
    if (masked) {
        if (!histogram.bins) {
            ...
            referenceHistogram = {
                ...histogram,
                bins: histogram.bins.map(({ values, exclude }) => ({
                    count: 0,
                    values,
                    exclude
                }))
    ...
    if (referenceHistogram && referenceHistogram.bins) {
        if (!masked || !selectionMasks) {
            ...
            selectionMasks.isEmptyByType(componentType) ||
    ...
    if (!selectionMasks) {
        ...
        selectionMasks = new DataframeMask(
            dataframe,
            selectionMasks.point,
            selectionMasks.edge,
            dataframe.lastMasks
        );
    }
...
function normalizeBinResult({ masked, dataType, histogram, selectionMasks }, binResult) {
    ...
    let maxElements = 0,
        ...
        castKeyToNumber =
            dataType === 'number' || (typeof minValue === 'number' && typeof maxValue === 'number');

    let binKeys,
        tag = (isMasked && selectionMasks && selectionMasks.tag) || undefined;

    if (binType === 'countBy' && isMasked === true) {
        binKeys = histogram.bins.map(({ values }) => values[0]);
    } else {
        binKeys =
            (binType === 'countBy' && bins && Object.keys(bins)) ||
            Array.from({ length: numBins }, (x, i) => i);
    }
    ...
    const binsNormalized = binKeys.map((key, index) => {
        ...
        if (binType !== 'nodata' && binType !== 'countBy' && binType !== 'histogram') {
            throw new Error('Unrecognized bin result type');
        }

        count = (binType !== 'nodata' && bins && typeof bins[key] === 'number' && bins[key]) || 0;
        ...
        if (binType === 'nodata') {
            ...
            binType === 'countBy' &&
            (value = binValues) &&
            (exclude = typeof value === 'object' && typeof value._other === 'object')
        ) {
            values = [key];
            ...
            values = value.isSingular ? [value.representative] : [value.min, value.max];
        } else {
            values = [minValue + index * binWidth, minValue + index * binWidth + binWidth];
        }
