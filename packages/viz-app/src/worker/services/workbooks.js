...
import {
  ...
  serializeWorkbook,
  workbook as createWorkbook
} from 'viz-app/models/workbooks';
...
function loadWorkbook(workbooksById, config, s3Cache) {
  return function loadWorkbookById({ workbookId, options = {} }) {
    ...
    const $workbook = Observable.from(downloadWorkbook(workbookId, s3Cache, config))
      .catch(e => {
...
function downloadWorkbook(workbookId, s3Cache, { S3, BUCKET }) {
  ...
  return Observable.defer(() => {
    return Observable.from(s3Cache.get(workbookURL))
      ...
      .catch(e =>
        Observable.from(s3.download(S3, BUCKET, workbookURL.pathname, { expectCompressed: true }))
          .do(() => log.debug('Got workbook from S3'))
          .mergeMap(response => s3Cache.put(workbookURL, response), (response, x) => response)
      )
...
export function saveWorkbookService(config, s3Cache) {
  return function saveWorkbook({ workbook }) {
    ...
    return Observable.from(s3Cache.put(workbookURL, serialized))
      .catch(e => {
