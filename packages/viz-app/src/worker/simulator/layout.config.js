...
Param.prototype.toClient = function(name, algoName) {
    return {
        ...
        type: this.type,
        value: this.toSlider(this.value)
    };
};
...
export function fromClient(controls, simControls) {
    var algoParams = _.object(
        _.map(controls.layoutAlgorithms, function(la) {
            return [la.algo.algoName, la.params];
        })
    );

    return _.object(
        _.map(simControls, function(update, algoName) {
            ...
            var cfg = _.object(
                _.map(update, function(val, paramName) {
