...
function getBufferBindings(simulator, stepNumber) {
  ...
  return {
    ...
    forwardsEdgeWeights: simulator.dataframe
      ...
      .then(obj => obj.buffer),
    forwardsEdgeStartEndIdxs: simulator.dataframe.getBuffer('forwardsEdgeStartEndIdxs', 'simulator')
      .buffer,
...
ForceAtlas2Barnes.prototype.initializeLayoutBuffers = function(simulator) {
  ...
  return Q.all([
    ...
    .fail(
      log.makeQErrorHandler(logger, 'Setting temporary buffers for barnesHutKernelSequence failed')
    );
};
