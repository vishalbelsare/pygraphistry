...
var kernelSpecs = {
    ...
    sort: {
        kernelName: 'sort',
        args: ['children', 'start', 'sort', 'count', 'bottom', 'numBodies', 'numNodes'],
...
function getBufferBindings(simulator, stepNumber) {
    return {
        ...
        backwardsEdgeWeights: simulator.dataframe
            ...
            .then(obj => obj.buffer),
        backwardsEdgeStartEndIdxs: simulator.dataframe.getBuffer(
            'backwardsEdgeStartEndIdxs',
            'simulator'
        ).buffer,
        ...
        forwardsEdgeWeights: simulator.dataframe
            .getClBuffer(simulator.cl, 'forwardsEdgeWeights', 'hostBuffer')
            .then(obj => obj.buffer),
        forwardsEdgeStartEndIdxs: simulator.dataframe.getBuffer(
            'forwardsEdgeStartEndIdxs',
            'simulator'
        ).buffer,
...
ForceAtlas2Barnes.prototype.initializeLayoutBuffers = function(simulator) {
    ...
    return Q.all([
        ...
        simulator.cl.createBuffer(
            num_work_groups * Float32Array.BYTES_PER_ELEMENT,
            'global_x_mins',
            ['mem_read_write', 'mem_host_no_access']
        ),
        simulator.cl.createBuffer(
            num_work_groups * Float32Array.BYTES_PER_ELEMENT,
            'global_x_maxs',
        ...
        simulator.cl.createBuffer(2 * numPoints * Float32Array.BYTES_PER_ELEMENT, 'curForces', [
            'mem_read_write',
            'mem_host_no_access'
        ]),
        simulator.cl.createBuffer(2 * numPoints * Float32Array.BYTES_PER_ELEMENT, 'prevForces', [
            'mem_read_write'
        ]),
        ...
        simulator.cl.createBuffer(numPoints * Float32Array.BYTES_PER_ELEMENT, 'tractions', [
            'mem_read_write'
        ])
    ])
        .spread(function(
        ...
        .fail(
            log.makeQErrorHandler(
                logger,
                'Setting temporary buffers for barnesHutKernelSequence failed'
            )
        );
};
...
ForceAtlas2Barnes.prototype.pointForces = function(simulator, workItems) {
    ...
    return this.toBarnesLayout
        .exec([workItems.toBarnesLayout[0]], [workItems.toBarnesLayout[1]])
        ...
        .then(() =>
            this.calculatePointForces.exec(
                [workItems.calculateForces[0]],
...
ForceAtlas2Barnes.prototype.edgeForces = function(simulator, workItemsSize) {
    ...
    return this.forwardsEdgeForceMapper
        .exec([workItemsSize.edgeForces[0]], [workItemsSize.edgeForces[1]])
        .then(() =>
            this.reduceForwardsEdgeForces.exec(
                [workItemsSize.segReduce[0]],
                [workItemsSize.segReduce[1]]
            )
        )
        .then(() =>
            this.backwardsEdgeForceMapper.exec(
                [workItemsSize.edgeForces[0]],
                [workItemsSize.edgeForces[1]]
            )
        )
        .then(() =>
            this.reduceBackwardsEdgeForces.exec(
                [workItemsSize.segReduce[0]],
                [workItemsSize.segReduce[1]]
            )
        );
...
ForceAtlas2Barnes.prototype.tick = function(simulator, stepNumber) {
    ...
    _.each(
        _.filter(this.kernels, function(x) {
            return x.argNames.indexOf('stepNumber') > 0;
        }),
        function(kernel) {
...
function getNumWorkitemsByHardware(deviceProps) {
    ...
    if (configOptions) {
        ...
        deviceProps.NAME.indexOf('GeForce GT 650M') != -1 ||
        ...
        deviceProps.NAME.indexOf('M290X') != -1 ||
    ...
    const gpuSizes = Object.entries(
        sizes
    ).reduce((result, [key, [numWorkGroups, workGroupSize]]) => {
