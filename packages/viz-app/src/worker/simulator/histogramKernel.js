...
HistogramKernel.prototype = {
    ...
    initializeBuffers: function(simulator) {
        const maxSizeInput = Math.max(
            simulator.dataframe.numPoints(),
            simulator.dataframe.numEdges()
        );
        ...
        return Q.all(that.qBuffers)
                ...
                (
                    ...
                    return Q.all([
                        ...
                        log.makeQErrorHandler(
                            logger,
                            'Writing to buffers for histogram kernel failed'
                        )
                    );
                }
            )
            .then(() => {
                ...
                return that.histogramKernel
                    ...
                    .then(() => {
                        const retOutput = new Int32Array(
                            MAX_NUM_BINS * Int32Array.BYTES_PER_ELEMENT
                        );

                        // TODO: Return all outputs, not just count;
                        return that.buffers.output
                            .read(retOutput)
                            .then(() => {
...
export default HistogramKernel;
