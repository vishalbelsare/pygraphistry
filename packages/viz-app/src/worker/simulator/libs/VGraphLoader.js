  ...
  return {
    ...
    edgeWeight: {
      load: function(values) {
        ...
        forwardsDesc.setDependencies([['__edgeWeights', 'edge'], ['forwardsEdges', 'hostBuffer']]);
...
function computeInitialPositions(vertexCount, edges, dimensions) {
  ...
  for (let i = 0; i < vertexCount; i++) {
    ...
    const vertex = [
      initSize * (offset.rowRollingSum + 0.9 * components[c].size * Math.random()) / vertexCount
    ];
    for (let j = 1; j < dimensions.length; j++) {
      vertex.push(
        initSize * (offset.rowYOffset + 0.9 * components[c].size * Math.random()) / vertexCount
      );
    }
