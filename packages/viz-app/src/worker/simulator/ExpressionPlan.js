...
PlanNode.prototype = {
  ...
  compile: function(generator, dataframe) {
    ...
    if (this.isConstant()) {
      ...
      const planNodeResult = generator.functionForPlanNode(this, dataframe.getColumnsByType());
  ...
  execute: function(dataframe, valuesRequired = false, iterationType = this.iterationType()) {
    if (iterationType !== this.iterationType()) {
      throw new Error(
        'Using an expression iterating over ' + this.iterationType() + ' within ' + iterationType
      );
    }
