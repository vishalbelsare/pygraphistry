...
function compileHelper({
    ...
    source,
    clContext,
    params: { loadtimeConstants, runtimeConstants }
}) {
    logger.trace('Compiling kernel', name);

    const prefix = Object.entries({ ...loadtimeConstants, ...runtimeConstants })
        .map(
            ([key, { val }]) =>
                typeof val === 'string' || typeof val === 'number' || val === true
                    ? '#define ' + key + ' ' + val
                    : val === null ? '#define ' + key : null
        )
        .filter(x => x)
        .join('\n');
...
function callHelper({ name, clContext }, kernel, workItems, buffers, workGroupSize) {
    ...
    logger.trace({ kernelName: name, workItems, workGroupSize }, 'Enqueuing kernel %s', name);
...
export default class Kernel {
    ...
    constructor(name, argNames, argTypes, file, clContext, loadtimeConstants = {}) {
        ...
        const params = {
            ...
            loadtimeConstants: Object.assign(
                { NODECL: { val: null } },
                ...argNames
                    .filter(arg => argTypes[arg] === cljs.types.define && arg in loadtimeConstants)
                    .map(arg => ({ [arg]: { val: loadtimeConstants[arg] } }))
            ),
            runtimeConstants: Object.assign(
                {},
                ...argNames
                    .filter(
                        arg => argTypes[arg] === cljs.types.define && !(arg in loadtimeConstants)
                    )
    ...
    setAllArgs(kernel) {
        ...
        argNames.filter(arg => arg in args).forEach((arg, i) => {
            try {
                ...
                if (dirty) {
                    logger.trace(
                        'Setting arg %d type %s of kernel %s to value %s',
                        i,
                        type,
                        name,
                        val
        ...
        Object.entries(newArgs).forEach(([arg, val]) => {
            if (arg in params.args) {
                ...
                if (val !== params.loadtimeConstants[arg].val) {
                    const msg = {
                        msg: `Kernel ${name} attempted a runtime override of loadtime argument ${arg}`,
                        name,
                        arg
                    };
                    logger.error(msg);
