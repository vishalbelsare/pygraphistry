 ...
 * @constructor
 */
function DataframeMask(
    dataframe,
    pointIndexes = undefined,
    edgeIndexes = undefined,
    basis = undefined
) {
    ...
    if (pointMask instanceof ArrayBuffer) {
        pointMask = new Uint32Array(pointMask);
    }
    if (edgeMask instanceof ArrayBuffer) {
        edgeMask = new Uint32Array(edgeMask);
    }
    ...
    if (mask !== undefined && basisMask !== undefined) {
        ...
        while (maskIndex < maskLength && basisIndex < basisLength) {
            if (mask[maskIndex] < basisMask[basisIndex]) {
                ...
                /* mask[maskIndex] === basisMask[basisIndex] */ localizedMask.push(basisIndex++);
                maskIndex++;
            }
        }
...
DataframeMask.unionOfTwoMasks = function(x, y) {
    ...
    while (xIndex < xLength && yIndex < yLength) {
        if (x[xIndex] < y[yIndex]) {
            ...
            /* x[xIndex] === y[yIndex] */ result[resultIndex++] = y[yIndex++];
            xIndex++;
        }
    }
...
DataframeMask.intersectionOfTwoMasks = function(x, y) {
    ...
    while (xIndex < xLength && yIndex < yLength) {
        if (x[xIndex] < y[yIndex]) {
            ...
            /* x[xIndex] === y[yIndex] */ result.push(y[yIndex++]);
            xIndex++;
        }
    }
    return result;
...
DataframeMask.complementOfMask = function(x, sizeOfUniverse) {
    ...
    while (complementIndex < sizeOfUniverse) {
        ...
        if (xIndex === xLength || x[xIndex] > complementIndex) {
            ...
            // Skip this X value which is <= the current complement.
            complementIndex++;
            xIndex++;
        }
    }
...
DataframeMask.minusMask = function(x, y, sizeOfUniverse) {
    ...
    if (x === undefined) {
        return DataframeMask.complementOfMask(y, sizeOfUniverse);
    }
...
DataframeMask.diffMask = function(x, y) {
    ...
    while (xIndex < xLength && yIndex < yLength) {
        if (x[xIndex] < y[yIndex]) {
            ...
            /* x[xIndex] === y[yIndex] */ result.intersection.push(y[yIndex++]);
            xIndex++;
        }
    }
    while (xIndex < xLength) {
...
DataframeMask.prototype = {
    numByType: function(type) {
        return this[type] !== undefined
            ? this[type].length
            : this.dataframe.getOriginalNumElements(type);
    },
    ...
    assertSameDataframe: function(other) {
        if (other !== undefined && other.dataframe !== this.dataframe) {
            throw new Error(
                'Support for working with masks across dataframes is not yet implemented.'
            );
        }
    },
    ...
    intersectionForType: function(type, other) {
        return DataframeMask.intersectionOfTwoMasks(
            this.getMaskForType(type),
            other.getMaskForType(type)
        );
    },
    ...
    complement: function() {
        const result = new DataframeMask(
            this.dataframe,
            DataframeMask.complementOfMask(
                this.getMaskForType('point'),
                this.dataframe.numPoints()
            ),
    ...
    minus: function(other) {
        ...
        return new DataframeMask(
            this.dataframe,
            DataframeMask.minusMask(
                this.getMaskForType('point'),
                other.getMaskForType('point'),
                this.dataframe.numPoints()
            ),
            DataframeMask.minusMask(
                this.getMaskForType('edge'),
                other.getMaskForType('edge'),
                this.dataframe.numEdges()
            )
        );
    },
    ...
    forEachIndexByType: function(type, iterator) {
        ...
        if (mask === undefined) {
            ...
            for (let i = 0; i < numElements; i++) {
                if (iterator.call(this, i, i) !== undefined) {
                    return;
                }
            }
        } else {
            for (let i = 0; i < numElements; i++) {
                if (iterator.call(this, mask[i], i) !== undefined) {
                    return;
                }
            }
        }
    ...
    fromJSON: function(clientMask) {
        if (clientMask === undefined) {
            return;
        }
        _.each(GraphComponentTypes, componentType => {
            if (clientMask[componentType] !== undefined) {
                const numComponents = this.dataframe.numByType(componentType);
                const componentMask = _.filter(
                    clientMask[componentType],
                    idx => idx < numComponents
                );
