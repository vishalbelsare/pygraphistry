...
function createCLContextNode(renderer, DEVICE_TYPE, vendor) {
    ...
    devices = devices.map(function(d) {
        ...
        var computeUnits = ocl
            .getDeviceInfo(d, ocl.DEVICE_MAX_WORK_ITEM_SIZES)
            .reduce(function(a, b) {
                return a * b;
            });
    ...
    if (desiredDevice) {
        ...
        if (devices.some(d => d.deviceType === 'GPU')) {
            devices = devices.filter(d => d.deviceType === 'GPU');
        }
        const startingDeviceIdx = (VIZ_LISTEN_PORT - devices.length % 2) % devices.length;
    ...
    for (i = 0; i < devices.length && deviceWrapper === null; i++) {
        ...
        try {
            ...
            if (ocl.VERSION_2_0) {
                wrapped.queue = ocl.createCommandQueueWithProperties(
                    wrapped.context,
                    wrapped.device,
                    []
                );
    ...
    if (deviceWrapper === null) {
        throw err !== null
            ? err
            : new Error('A context could not be created from an available device');
    }
...
var compile = Q.promised(function(cl, source, kernels) {
    ...
    try {
        ...
        ocl.buildProgram(
            program,
            [cl.device],
            '-I ' + includeDir + ' -cl-fast-relaxed-math ' + clver
        );
