...
function VizServer(app, socket, cachedVBOs, loggerMetadata, colorTexture) {
    ...
    this.socket.once('begin_streaming', (ignore, cb) => {
        this.renderConfig.take(1).subscribe(renderConfig => {
...
VizServer.prototype.beginStreaming = function(renderConfig, colorTexture, app) {
    ...
    const activeBuffers = _.chain(renderConfig.models)
        .pairs()
        .filter(pair => {
            const model = pair[1];
    ...
    const activeTextures = _.chain(renderConfig.textures)
        .pairs()
        .filter(pair => {
    ...
    this.socket.on('planned_binary_requests', request => {
        ...
        logger.trace(
            { buffers: request.buffers, textures: request.textures },
            'Client sending planned requests'
        );
    ...
    clientReady.subscribe(
        logger.debug.bind(logger, 'CLIENT STATUS'),
        log.makeRxErrorHandler(logger, 'clientReady')
    );
            ...
            return (
                driver
                    ...
                    .do(VBOs => {
                        logger.trace(
                            { step: step },
                            '1. pre-fetched VBOs for xhr2: ' +
                                sizeInMBOfVBOs(VBOs.compressed) +
                                'MB'
                        );
                        ...
                        this.lastMetadata = {
                            elements: VBOs.elements,
                            bufferByteLengths: VBOs.bufferByteLengths
                        };
                    ...
                    .flatMap(VBOs => {
                        ...
                        return clientReady
                            ...
                            .do(() => {
                                logger.trace(
                                    { step: step },
                                    '2b. Client ready, proceed and mark as processing.'
                                );
                    ...
                    .flatMap(VBOs => {
                        ...
                        app.bufferTransferFinisher = function(bufferName) {
                            ...
                            if (transferredBuffers.length === requestedBuffers.length) {
                                ...
                                logger.trace(
                                    'Socket',
                                    '...client asked for all buffers in ' + elapsedTime + 'ms'
                                );
                            }
                        };
                        ...
                        const receivedAll = colorTexture
                            .flatMap(eachColorTexture => {
                            ...
                            .do(clientElapsed => {
                                ...
                                logger.trace(
                                    'Socket',
                                    '...client received all buffers in ' + clientElapsed + 'ms'
                                );
                            });
                    ...
                    .flatMap(() => {
                        logger.trace(
                            '7. Wait for next animation step, updateVboSubject, or if we are behind on ticks'
                        );

                        const filteredUpdateVbo = this.updateVboSubject.filter(_.identity);

                        const behindOnTicks = graphObservable
                            .take(1)
                            .filter(latestGraph => latestGraph.simulator.versions.tick > lastTick);

                        return Rx.Observable
                            .merge(this.ticksMulti, filteredUpdateVbo, behindOnTicks)
                            .take(1)
                            .do(() => {
...
VizServer.prototype.dispose = VizServer.prototype.unsubscribe = function() {
