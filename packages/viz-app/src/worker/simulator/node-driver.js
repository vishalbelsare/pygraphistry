...
function graphCounts(graph, debug = false) {
    ...
    const selectedPointIndexes = {
        num: numSelectedPointIndexes,
        offset: offsetSelectedPointIndexes
    };
...
export function create(dataset, socket, nBodyInstance) {
    ...
    const graph = nBodyInstance
        ...
        .spread(graph => {
            ...
            const outDegrees = graph.simulator.dataframe.getHostBuffer('forwardsEdges')
                .degreesTyped;
            const inDegrees = graph.simulator.dataframe.getHostBuffer('backwardsEdges')
                .degreesTyped;
                ...
                .map(compressedVBOs => {
                    ...
                    const uncompressed = _.object(
                        _.zip(
                            ...
                            bufferNames.map(
                                (_, i) =>
                                    compressedVBOs[i].buffer.buffer || compressedVBOs[i].buffer
                            )
                        )
                    );
