...
ExpressionCodeGenerator.prototype = {
        ...
        const methodCall = (firstArg, outputFunctionName, restArgs) => {
            return this.wrapSubExpressionPerPrecedences(
                firstArg +
                    ...
                    outputFunctionName +
                    '(' +
                    (restArgs !== undefined ? restArgs.join(', ') : '') +
                    ')',
                precedence,
                outerPrecedence
            );
    ...
    transformASTForNullGuards: function(ast, attributeData, dataframe) {
        if (this.isPredicate(ast)) {
            ...
            _.each(attributeData, attributeName => {
                const nativeType = dataframe.getDataType(
                        attributeName.attribute,
                        attributeName.type
                    ),
                    identifier = {
                        type: 'Identifier',
                        name: attributeName.type + ':' + attributeName.attribute
                    };
    ...
    regularExpressionLiteralFromLikePattern: function(pattern, escapeChar) {
        ...
        for (let i = 0; i < matches.length; i++) {
            ...
            if (matchString.length === 2) {
                ...
                throw Error(
                    'Unrecognized match for LIKE placeholders: ' +
                        matchString +
                        ' at: ' +
                        match.index
                );
        ...
        if (lastMatchIndex < pattern.length) {
            ...
            outputLiteralString = outputLiteralString.concat(
                escapeRegexNonPattern(lastPatternSegment)
            );
        }
    ...
    expressionForConditions: function(cases, elseClause, bindings, depth) {
        ...
        _.each(cases, caseExpr => {
            const conditionArg = this.expressionStringForAST(
                    caseExpr.condition,
                    bindings,
                    depth,
                    precedence
                ),
                resultArg = this.expressionStringForAST(
                    caseExpr.result,
                    bindings,
                    depth,
                    precedence
    ...
    planNodeExpressionStringForAST: function(ast, bindings, depth, outerPrecedence) {
        ...
        const localizedArgs = _.mapObject(
            _.pick(localizedAST, this.inputPropertiesFromAST(ast)),
            arg => {
        ...
        switch (ast.type) {
            case 'NotExpression':
                ...
                return {
                    ast: localizedAST,
                    expr: this.wrapSubExpressionPerPrecedences(
                        subExprString,
                        precedence,
                        outerPrecedence
                    )
            ...
            case 'BinaryPredicate':
                switch (ast.operator.toUpperCase()) {
                    case 'AND':
                        subExprString =
                            localizedArgs.left + '.intersection(' + localizedArgs.right + ')';
                        return {
                            ast: localizedAST,
                            expr: this.wrapSubExpressionPerPrecedences(
                                subExprString,
                    ...
                    default:
                        return {
                            ast: ast,
                            expr: this.expressionStringForAST(
                                ast,
                                bindings,
                                depth2,
                                outerPrecedence
            ...
            case 'Identifier':
                return {
                    ast: ast,
                    expr: this.expressionStringForAST(ast, bindings, depth2, outerPrecedence)
                };
            default:
    ...
    expressionStringForAST: function(ast, bindings, depth = 0, outerPrecedence = undefined) {
        ...
        switch (ast.type) {
            ...
            case 'BetweenPredicate':
                ...
                subExprString =
                    args.value + ' >= ' + args.start + ' && ' + args.value + ' <= ' + args.stop;
                return this.wrapSubExpressionPerPrecedences(
                    subExprString,
                    precedence,
                    outerPrecedence
                ...
                switch (ast.operator.toUpperCase()) {
                    case 'LIKE': {
                        ...
                        if (
                            ...
                            precedence = this.precedenceOf('!==');
                            subExprString =
                                arg + '.indexOf(' + literalExpressionFor(substring) + ') !== -1';
                        } else if (pattern.indexOf('%') !== pattern.lastIndexOf('%')) {
                            return this.regexExpressionForLikeOperator(
                                ast,
                                bindings,
                                depth,
                                outerPrecedence
                            ...
                            if (index === -1) {
                                ...
                                subExprString =
                                    arg +
                                    '.endsWith(' +
                                    literalExpressionFor(suffix) +
                                    ') && ' +
                                    arg +
                                    '.startsWith(' +
                                    literalExpressionFor(prefix) +
                                    ')';
                            }
                        }
                        return this.wrapSubExpressionPerPrecedences(
