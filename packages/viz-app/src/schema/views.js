...
export function views(path, base) {
  return function views({
    ...
    const setPruneOrphansAndMaskDataframe = setHandler(
      path,
      loadViewsById,
      ...
      return loadViewsById({
        ...
        return [
          ...
          ...Object.keys(column).map(key =>
            $value(`${viewPath}.columns[${columnIndex}]['${key}']`, column[key])
          )
        ];
      });
    }
    ...
    function movePointSelection(path, [{ x = 0, y = 0 } = {}]) {
      ...
      return moveSelectedNodes({
        ...
        viewIds,
        coords: { x, y }
      }).map(({ workbook, view, points }) =>
        $value(
          `workbooksById['${workbook.id}']` + `.viewsById['${view.id}']` + `.selection.point`,
          $atom(points)
        )
      );
    }
