...
export function scene(path, base) {
    return function scene({ tickLayout, loadViewsById, loadLabelsByIndexAndType }) {
        ...
        const setSimulating = setHandler(
            path,
            loadViewsById,
            (scene, key, simulating, path, { view }) =>
                Observable.defer(() => {
                    return tickLayout({ view, play: !!simulating, layout: !!simulating }).concat(
                        Observable.of({
                            path,
                            value: (scene[key] = simulating)
                        })
                    );
                })
        );
        const setColors = setHandler(path, loadViewsById, (node, key, color, path, { view }) =>
            Observable.defer(() => {
                ...
                if (type === 'foreground' && nBody) {
                    ...
                    return tickLayout({ view, layout: false }).concat(
                        Observable.of({
                            path,
                            value: color
                        })
