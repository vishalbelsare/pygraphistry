...
export function expressions(path, base) {
  return function expressions({ loadViewsById, maskDataframe, updateExpressionById }) {
    ...
    const batchSetValues = ({ viewId, workbookId, expressionId, path, values }) =>
      Observable.from(Object.keys(values))
        .mergeMap(
          key =>
          ...
          return Observable.from(
            ...
            maskDataframe({ view })
              ...
              .mergeMap(({ view }) => {
                ...
                if (expression.enabled === true || 'enabled' in expressionProps) {
  ...
  return function removeExpressionHandler(path, [itemId]) {
    ...
    return removeItem({
      ...
      while (++index < listLen) {
        if (found) {
          pathValues.push(
            $value(`${viewPath}['${listName}'][${index - 1}]`, (list[index - 1] = list[index]))
          );
          continue;
        }
        let itemRef = list[index],
          itemRefPath;
