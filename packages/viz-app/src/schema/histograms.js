  ...
  return function histograms({
    ...
    function getHistogramTypeReference(path) {
      ...
      return histogramIds.reduce((values, histogramId) => {
        return histogramTypes.reduce((values, histogramType) => {
          const refPath =
            histogramType === 'global'
              ? basePath.concat('histogramsById')
              : basePath.concat('selection', 'histogramsById');
          refPath.push(histogramId);
          values.push(
            $value(basePath.concat('histogramsById', histogramId, histogramType), $ref(refPath))
          );
          return values;
        }, values);
      }, []);
    ...
    function getSelectionHistograms(path) {
      ...
      return loadSelectionHistogramsById({
        ...
        ({ workbook, view, histogram }) =>
          getHandler([], () => Observable.of(histogram)).call(this, restPath),
        ({ workbook, view, histogram }, { path, value }) =>
          $value(
            [
              'workbooksById',
              workbook.id,
    ...
    function updateHistogramFilterHandler(path, binIndexes = []) {
      ...
      return loadHistogramsById({
        ...
        let filter,
          query = histogramFilterQuery(histogram, binIndexes);

        if (filterRef) {
          filter = expressionsById[filterRef.value[filterRef.value.length - 1]];
        }

        // Filter arguments that are out of bounds.
        binIndexes = binIndexes.filter(index => index >= 0 && index < bins.length && bins[index]);
        ...
        return Observable.of(
          ...
          maskDataframe({ view })
            ...
            .mergeMap(() => {
              ...
              if (selection && selection.mask && selection.type === 'window') {
                pathValues.push(
                  $invalidate(`
      ...
      return loadHistogramsById({
        ...
        .mergeMap(({ workbook, view, histogram, maskedComponents: masked = {} }) => {
          ...
          const values = Object.keys(
            masked
          ).map((componentType, x, y, elements = masked[componentType]) =>
            $value(
              `
                        workbooksById['${workbook.id}']
                            .viewsById['${view.id}']
              ...
              $atom(
                ((darken = darken || (elements && elements.length > 0)) && elements) || elements
              )
            )
          );
