...
export function and (...args) {
    return args.reduce((left, right) => ({
        type: 'BinaryPredicate', operator: 'AND', left, right
    }));
}
export function or (...args) {
    return args.reduce((left, right) => ({
        type: 'BinaryPredicate', operator: 'OR', left, right
    }));
}
...
export function printAST (ast) {
    ...
    switch (ast.type) {
        case 'BetweenPredicate':
            ...
            return [
                properties.value, 'BETWEEN',
                properties.start, 'AND',
                properties.stop
            ].join(' ');
        case 'RegexPredicate':
        case 'LikePredicate':
