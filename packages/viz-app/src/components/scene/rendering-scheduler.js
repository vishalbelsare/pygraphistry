        ...
        const loop = () => {
            ...
            if (renderTaskNames.length === 0) {
                ...
                if (shouldUpdateRenderTime) {
                    ...
                    if (timeDelta > PAUSE_RENDERING_DELAY) {
                        ...
                        if (tag === undefined) {
                            return pauseRenderingLoop(nextFrameId);
                        }
                        quietRenderQueue = Object.create(null);
                    }
                }
                return nextFrameId = window.requestAnimationFrame(loop);
            }
            ...
            if (tagsWithRenderFull.length > 0) {
                ...
                for (let i = -1, n = tagsWithRenderFull.length; ++i < n;) {
                    ...
                    quietRenderQueue[tag] = renderQueue[tag];
            ...
            nextFrameId = window.requestAnimationFrame(loop);
        }
...
RenderingScheduler.prototype.attemptToAllocateBuffersOnHints = function (config, renderState, { edges = 0, points = 0 } = {}) {
    ...
    this.add(this.hintsAllocationCycle = Observable
...
RenderingScheduler.prototype.renderSlowEffects = function () {
    ...
    var expanded, renderer = this.renderer;
...
RenderingScheduler.prototype.allocateAllArrayBuffersFactory = function (config, numElements, renderState) {
    var functions = [], renderer = this.renderer;
...
RenderingScheduler.prototype._unsubscribe = function() {
