...
class Scene extends React.Component {
  ...
  constructor(props, context) {
    ...
    const makeEventHandler = eventName =>
      (this[eventName] = event => {
        ...
        if (!dispatch || !renderState || !renderingScheduler) {
          return;
        }
        const { camera } = renderState;
    ...
    return (
      <div
          ...
          <div className={styles['top-level-scene-info']}>
            {(info && (
              <table className={styles['meter']}>
                <tbody>
                  <tr>
                    <td className={styles['meter-label']}>nodes</td>
                    <td className={styles['meter-value']}>{(points && points.elements) || 0}</td>
                  </tr>
                  <tr>
                    <td className={styles['meter-label']}>edges</td>
                    <td className={styles['meter-value']}>{(edges && edges.elements) || 0}</td>
                  </tr>
                </tbody>
              </table>
            )) ||
        ...
        <div className={styles['logo-container']}>
          {(showLogo && <img draggable="false" src="img/logo_white_horiz.png" />) || undefined}
          <div
            className={styles['logo-version']}
  ...
  setupDOMAndSourceListeners(props = {}, state = {}) {
    ...
    let { play = 5000 } = props;
    const { socket, simulation, selectToolbarItem } = props;
    let {
      renderer,
      renderState,
      scrollSource,
    ...
    if (!container || !simulation || !renderState || (hasDOMListeners && hasSourceListeners)) {
      return;
    }
    ...
    if (play === true) {
      ...
      stopAutoPlay = Observable.race(
        ...
        Observable.defer(() => Gestures.start(container)).catch(() => Observable.empty())
      ).take(1);
    }
