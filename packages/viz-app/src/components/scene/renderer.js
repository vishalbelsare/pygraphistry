...
export function init(config, canvas, urlParams) {
     ...
     */
    const getUniformLocationFast = addressMemoizer(
        uniformLocations,
        'uniform',
        'getUniformLocation'
    );
    ...
    subscription.add(
        Observable.fromEvent(canvas, 'webglcontextlost').subscribe(() =>
    ...
    function addressMemoizer(cache, cacheName, glLocationMethodName) {
        return function(gl, program, programName, address) {
            ...
            if (cache[programName][address] === -1) {
                throw new Error(
                    'Error binding address ' + cacheName + '::' + programName + '::' + address
                );
            }
    ...
    */
    ...
    function createCamera(state, pixelRatio, urlParams = {}) {
        ...
        const mode = urlParams.camera
            ? urlParams.camera === '3d' ? '3d' : '2d'
            : camConfig.type === '3d' ? '3d' : '2d';
    ...
    function getTextureDims(config, canvas, camera, name) {
        ...
        //Retina-quality
        const width = textureConfig.hasOwnProperty('width')
            ? Math.floor(0.01 * textureConfig.width.value * canvas.width)
            : Math.floor(canvas.width);
        const height = textureConfig.hasOwnProperty('height')
    ...
    function createRenderTargets(config, neededTextures, canvas, gl, camera) {
        ...
        _.zip(
            ...
            initializeTexture(
                ...
                fbo,
                renderBuffer,
                eachDimensions.width,
                eachDimensions.height
            );
        });
    ...
    function setGlOptions(gl, glOpts) {
        ...
        _.each(glOpts || {}, (optionCalls, optionName) => {
            if (whiteList[optionName] !== true || typeof gl[optionName] !== 'function') {
                return;
            }
    ...
    function createPrograms(config, gl) {
        ...
        return Object.keys(config.programs).reduce((programs, programName) => {
            ...
            if (
                !(gl.isShader(vertShader) && gl.getShaderParameter(vertShader, gl.COMPILE_STATUS))
            ) {
                log = gl.getShaderInfoLog(vertShader);
                debug('Error in vertex shader', programName, log, sourceCode);
                throw new Error(
                    'Could not compile vertex shader "' + programName + '" (' + log + ')'
                );
            }
            ...
            if (
                !(gl.isShader(fragShader) && gl.getShaderParameter(fragShader, gl.COMPILE_STATUS))
            ) {
                log = gl.getShaderInfoLog(fragShader);
                debug('Error in fragment shader', programName, log, sourceCode);
                throw new Error(
                    'Could not compile fragment shader "' + programName + '" (' + log + ')'
                );
            }
            ...
            if (!gl.getProgramParameter(program, gl.VALIDATE_STATUS)) {
                ...
                throw new Error(
                    "Could not validate GL program '" + programName + "':",
                    JSON.stringify(programOptions)
                );
            }
    ...
    function createBuffers(config, gl, bufferSizes, hostBuffers) {
        return Object.keys(config.models).reduce((buffers, name) => {
    ...
    function loadBuffers(state, bufferData) {
        ...
        _.each(bufferData, (data, bufferName) => {
            debug(
                'Loading buffer data for buffer %s (data type: %s, length: %s bytes)',
                bufferName,
            ...
            if (model === undefined) {
                console.error(
                    "Asked to load data for buffer '%s', but corresponding model not found",
                    bufferName
                );
            ...
            if (typeof buffers[bufferName] === 'undefined') {
                console.error(
                    "Asked to load data for buffer '%s', but no buffer by that name exists locally",
                    bufferName,
                    buffers
                );
                return false;
    ...
    function loadBufferBody(state, buffer, bufferName, model, data, bufferSizes, gl) {
        ...
        if (bufferSizes[bufferName] >= data.byteLength) {
            debug(
                'Reusing existing GL buffer to load data for buffer %s (current size: %d, new data size: %d)',
                bufferName,
                bufferSizes[bufferName],
                data.byteLength
            );
            gl.bufferSubData(glArrayType, 0, data);
        } else {
            debug(
                'Creating new buffer data store for buffer %s (new size: %d, hint: %s)',
                bufferName,
                data.byteLength,
                glHint
            );
    ...
    function updateIndexBuffer(state, length, repetition) {
        ...
        if (oldHostBuffer.length < length * repetition) {
            const longerBuffer = expandHostBuffer(
                gl,
                length,
                repetition,
                indexHostBuffers[repetition]
            );
            ...
            debug(
                'Expanding index buffer',
                glBuffer,
                'memcpy',
                oldHostBuffer.length / repetition,
                'elts',
                'write to',
                length * repetition
            );
    ...
    function render(
        ...
        if (texturesToRead.length > 0) {
            _.uniq(texturesToRead).forEach(renderTarget => {
                ...
                const readDims = readPixelsOverride || {
                    x: 0,
                    y: 0,
                    width: dims.width,
                    height: dims.height
                ...
                if (!texture || texture.length !== readDims.width * readDims.height * 4) {
                    debug(
                        'reallocating buffer',
                        texture.length,
                        readDims.width * readDims.height * 4
                    );
    ...
    function renderItem(
        ...
        const textureToRead =
            renderTarget !== null && itemDef.readTarget ? renderTarget : undefined;
        ...
        if (typeof numElements === 'undefined' || numElements === 0) {
            debugRenderItem(
                'Not rendering item "%s" because it doesn\'t have a non-zero numElements',
                item
            );
        ...
        setGlOptions(
            gl,
            _.omit(_.extend({}, state.options, itemDef.glOptions), 'clearColor', 'lineWidth')
        );
    ...
    function getServerBufferNames(config) {
        return getBufferNames(config, modelAttr => {
    ...
    function getActiveIndices(config) {
        ...
        const activeIndexModesLists = renderItems.map(itemName => {
            ...
            return _.pairs(bindings)
                ...
                .map(datasource => {
                    return datasource === 'VERTEX_INDEX' ? 1 : datasource === 'EDGE_INDEX' ? 2 : 0;
                })
