...
const Paginate = mapPropsStream((props) => {
    ...
    return props.switchMap(selectPage, (props, page) => ({
        ...props, page, onNext, onPrev, onLast, onFirst, onSelect
    }));
...
function Pagination({ page = 1, pages = 0,
                      ...
                      onNext, onPrev, onLast, onFirst, onSelect, ...props }) {

    const buttons = [];
    const numControls = prev + next + boundaryLinks * 2;
    const numButtons = Math.min(pages, typeof size === 'number' ?
    ...
    do {
        ...
        const isEllipsisElement = (ellipsis === true && numButtons > 4) && (
                                  ((hasHiddenPagesBefore && count === 1) === true) ||
                                  ((hasHiddenPagesAfter  && count === numButtons - 2) === true));

        const event = isLastElement ? { to: pages } :
                     isFirstElement ? { to: 1     } :
                 !isEllipsisElement ? { to: index } :
