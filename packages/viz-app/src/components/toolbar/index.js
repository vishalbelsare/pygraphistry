...
export function ButtonListItems({ id, name, children }) {
    return (
        <ButtonToolbar
            ...
            <ButtonGroup vertical={id === 'camera'}>{children}</ButtonGroup>
        </ButtonToolbar>
    );
}

function ButtonListItemTooltip(name) {
    return <Tooltip id="button-list-item-tooltip">{name}</Tooltip>;
}
...
export function ButtonListItem({
    ...
    const placement =
        groupId === 'camera' || (window && window.innerWidth < 330) ? 'right' : 'bottom';
