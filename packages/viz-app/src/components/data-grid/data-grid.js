...
const MemoizeStyleProps = mapPropsStream(props => {
  ...
  return props.scan((prev, props) => {
    ...
    if (scrollTop !== prev.scrollTop || scrollLeft !== prev.scrollLeft) {
...
function DataGrid(props) {
  ...
  if (renderColHeaderCell) {
      ...
      thead.push(
        <th key="grid-corner-cell">
          <div style={rowCellStyle}>
            <div className={styles['grid-head-row-cell-content']}>{cell}</div>
            <div className={styles['grid-head-row-cell-wrapper']}>
              <div
                style={cornerStyle}
                className={`${styles['grid-head-col-cell']} ${styles[
                  'grid-head-row-cell'
    ...
    while (++colIndex < colsPerPage) {
      ...
      thead.push(
        <th data-col-index={col} onClick={onColHeaderSelect} key={`grid-col-${col}-cell`}>
          <div style={colCellStyle}>
            <div style={colHeaderStyle} className={styles['grid-head-col-cell']}>
  ...
  while (++rowIndex < rowsPerPage) {
    ...
    cells = new Array(
      Number(Boolean(renderRowHeaderCell)) + Number(Boolean(renderCell)) * colsPerPage
    );
    if (renderRowHeaderCell) {
      cell = renderRowHeaderCell(row, props);
      cells.push(
        <td key={`grid-row-${row}-cell`} className={styles['grid-head-cell']}>
          <div style={rowCellStyle}>
            <div className={styles['grid-head-row-cell-content']}>{cell}</div>
            <div className={styles['grid-head-row-cell-wrapper']}>
              <div
                style={rowHeaderStyle}
                className={`${styles['grid-head-row-cell']} ${row % 2
                  ? styles['grid-cell-odd']
    ...
    if (renderCell) {
      while (++colIndex < colsPerPage) {
        col = startCol + colIndex;
        cells.push(
          <td onClick={stopPropagationIfAnchor} key={`grid-col-${col}-row-${row}-cell`}>
            <div
              style={cellStyle}
              className={`${styles['grid-cell']} ${row % 2
                ? styles['grid-cell-odd']
                : styles['grid-cell-even']}`}>
  ...
  return (
    <div className={styles['grid-scroller-container']}>
      <div
        className={styles['grid-scroller']}
        onWheel={onWheel}
        onScroll={onScroll}
        ...
        <div style={tableContainerStyle}>
          <table className={styles['grid']} style={tableStyle}>
            <tbody style={tbodyStyle}>{tbody}</tbody>
