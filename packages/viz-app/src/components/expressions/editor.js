...
class DataframeCompleter {
    constructor(templates = []) {
         ...
         */
        this.templates = Object.keys(
            templates.reduce((templates, { name, identifier }) => {
                templates[name] = true;
...
class InlineAnnotation {
    ...
    update() {
        var anchorRange = AceRange.fromPoints(
            this.startAnchor.getPosition(),
            this.endAnchor.getPosition()
        );
        if (this.marker) {
        ...
        if (this.info.text) {
            this.marker = this.session.addMarker(
                anchorRange,
                clazz,
                (stringBuilder, range, left, top, config) => {
...
function clearAnnotationsAndMarkers(session) {
    session.getAnnotations().forEach(annotation => annotation.remove());
