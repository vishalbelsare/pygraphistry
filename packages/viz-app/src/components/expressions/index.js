...
const expressionTooltip = <Tooltip id="expression-tooltip">Expression</Tooltip>;

const expressionEnabledTooltip = <Tooltip id="expression-enabled-tooltip">Enabled</Tooltip>;

const deleteExpressionTooltip = <Tooltip id="delete-expression-tooltip">Delete Expression</Tooltip>;
...
function TemplateOptionRenderer({ name, dataType, componentType, showDataTypes }) {
  return (
    <span>
      ...
      <label>{name}</label>
      {showDataTypes && <span style={{ fontStyle: 'italic', marginLeft: '5px' }}>{dataType}</span>}
    </span>
  );
}

export function ExpressionTemplates({
  name = 'Expressions',
  templates = [],
  ...
  return (
    <Select
      ...
      options={templates.map(({ name, dataType, identifier, componentType }, index) => ({
        value: index,
...
export function ExpressionItem({
  ...
  return (
    <Row className={styles['expression-row'] + ' ' + styles['expression-item']}>
      <Col xs={12} md={12} lg={12} style={(!isSystem && { paddingRight: 0 }) || undefined}>
        <OverlayTrigger
          placement="top"
          trigger={!readOnly ? ['hover'] : []}
          overlay={expressionTooltip}>
          <div
            className={classNames({ [styles['read-only']]: readOnly })}
      ...
      {!isSystem && (
        <Col
          ...
          md={4}
          lg={4}
          className={styles['expression-row']}
          style={{ padding: 0, transform: 'scale(0.9)' }}>
          <Col xs={6} md={6} lg={6}>
