...
const globalMousePosition = Observable.defer(() => Gestures.move())
    ...
    .distinctUntilChanged((a, b) => a.clientX === b.clientX && a.clientY === b.clientY)
...
const WithPointsAndMousePosition = mapPropsStream(props =>
    props
        .combineLatest(
            sceneUpdates,
            globalMousePosition,
...
class Labels extends React.Component {
    ...
    render() {
        let camera, canvas, matrix, pointSize, pixelRatio, scalingFactor;
        let {
            mouseX,
            mouseY,
            onLabelsUpdated,
        ...
        while (++labelIndex < labelCount) {
            ...
            if (
                ...
                (label.index === undefined || label.title === undefined)
            ) {
                continue;
            }
            ...
            const size =
                type === 'edge'
                    ? 0 // Clamp like in pointculled shader
                ...
                childrenToRender.push(
                    React.cloneElement(child, {
                        ...
                        style: {
                            ...
                            transform: `${`translate3d(${x}px, ${y}px, 0)` /* force hardware acceleration */} ${`perspective(1000px)` /* force sub-pixel font rendering*/}`
                        }
                    })
                );
            }
        ...
        return (
            <div
                className={classNames({
                    ...
                    [styles['labels-container-dark']]: isDark(
                        Color((renderer.background || {}).color).rgb()
                    )
                })}>
                {childrenToRender}
            </div>
    ...
    updateLabelSettings(currProps, nextProps) {
        const { falcor, enabled, poiEnabled, renderState, renderingScheduler } = nextProps;
        if (
            !falcor ||
            !renderState ||
            !renderingScheduler ||
            (currProps.enabled === enabled && currProps.poiEnabled === poiEnabled)
        ) {
            return;
        }
        labelSettings.next({
