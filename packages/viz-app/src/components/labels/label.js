...
export class Label extends React.Component {
  ...
  render() {
    ...
    const iconSize =
      type === 'edge' ? 30 : Math.max(5, Math.min(scalingFactor * sizes[index], 50)) / pixelRatio;
...
function PointIcon({ iconClass, pointColor, pointRgb, iconSize, type }) {
  ...
  return (
    <div
      style={{
        transform: `${`scale3d(${c},${c},${c})`} ${`translate3d(-100%, 0, 0)` /* force hardware acceleration */} ${`perspective(0px)` /* force sub-pixel font rendering when scaled */}`
      }}
      className={classNames({
        [styles['point-icon-container']]: true,
        [styles['light-color']]: !isDark(pointRgb)
      })}>
      <div className={[styles['point-icon']]} style={{ backgroundColor: pointColor }}>
        <i
          className={classNames({
            fa: true,
            'fa-fw': true,
...
function LabelTitle({
  ...
  if (!showFull) {
    return (
      <div className={styles['label-title']} onMouseDown={onMouseDown} onTouchStart={onTouchStart}>
        <span onMouseDown={stopPropagationIfAnchor} className={styles['label-title-text']}>
  ...
  return (
    <div onMouseDown={onMouseDown} onTouchStart={onTouchStart} className={styles['label-title']}>
      <a
        className={classNames({
      ...
      <OverlayTrigger
        ...
        placement="bottom"
        overlay={
          <Tooltip className={styles['label-tooltip']} id={`tooltip:title:${type}:${title}`}>
            Exclude if "{type}:_title = {<span dangerouslySetInnerHTML={titleExcludeHTML} />}"
          </Tooltip>
        }>
        <a
...
const operatorForColumn = (function(operators) {
  return (queryType, dataType) => {
    return (
      operators[queryType + '_' + dataType] ||
      (operators[queryType + '_' + dataType] =
    ...
    <tr className={styles['label-pair']}>
      ...
      <td className={important ? styles['label-value-important'] : styles['label-value']}>
        <div className={styles['label-value-wrapper']}>
          <span onMouseDown={stopPropagationIfAnchor} className={styles['label-value-text']}>
          ...
          <div className={styles['label-icons']}>
            <OverlayTrigger
              ...
              <a
                className={styles['exclude-by-key-value']}
                onMouseDown={stopPropagation}
                onClick={preventPropagation(
                  () =>
                    onExclude &&
            ...
            <OverlayTrigger
              ...
              <a
                className={styles['filter-by-key-value']}
                onMouseDown={stopPropagation}
                onClick={preventPropagation(
                  () =>
                    onFilter &&
