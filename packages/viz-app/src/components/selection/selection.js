...
import { curPoints, pointSizes, cameraChanges, hitmapUpdates } from 'viz-app/client/legacy';
  ...
  return (
    <div
      ...
      style={{
        width: br.x - tl.x || 0,
        height: br.y - tl.y || 0,
        transform: `translate3d(${tl.x || 0}px, ${tl.y || 0}px, 0)`
      }}
    />
  );
}
...
function HighlightPoint({ index, sizes, points, renderState, selectedPoints, onPointSelected }) {
  ...
  const size = Math.max(5, Math.min(scalingFactor * sizes[index], 50)) / camera.pixelRatio || 0;
  ...
  return (
    <div
      ...
      style={{
        width: `${hitArea}px`,
        height: `${hitArea}px`,
        transform: `translate3d(${x - hitArea * 0.5}px, ${y - hitArea * 0.5}px, 0)`
      }}>
      <div
        className={styles['selection-point-center']}
        style={{
          borderRadius: size * 0.5,
          width: size,
          height: size,
          transform: `translate3d(${(hitArea - size) * 0.5}px, ${(hitArea - size) * 0.5}px, 0)`
        }}
      />
    </div>
  );
...
const WithPointsAndSizes = mapPropsStream(props =>
  props
    .combineLatest(
      cameraChanges
        .merge(hitmapUpdates)
...
function onMaskTouchStart(event) {
  const { mask, dispatch, simulating, renderState, renderingScheduler } = onMaskTouchStart;

  if (simulating || !dispatch || !renderState || !renderingScheduler) {
    return;
  }
