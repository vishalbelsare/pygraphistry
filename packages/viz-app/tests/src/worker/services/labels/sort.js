...
describe('sort', () => {
    ...
    it('handles event nodes - EventID', done => {
        ...
        const expectedOrder = [
            mostImportantEventKeys[0],
            mostImportantEventKeys[1],
            mostImportantEventKeys[2]
        ]
    ...
    it('handles entities', done => {
        ...
        const expectedOrder = [
            mostImportantEntityKeys[0],
            mostImportantEntityKeys[1],
            mostImportantEntityKeys[2]
        ]
            .map((k, i) => ({ key: k, value: i }))
            .concat([key]);
        const reverse = expectedOrder.slice().reverse();
