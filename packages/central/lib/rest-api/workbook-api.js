...
export function initWorkbookApi(app, config) {
    ...
    app.post('/workbook', bodyParser.json({ limit: '10MB' }), (req, res) =>
        uploadWorkbookRequestHandler(req, res, localCache, config)
    );

    app.get('/workbook/:workbook', (req, res) =>
        downloadWorkbookRequestHandler(req, res, localCache, config)
    );
}
    ...
    return saveWorkbook({ workbook, config, localCache })
        ...
        .subscribe(
            () => {
                ...
                logger.info(
                    { req, res, workbook: workbook.id },
                    `Uploaded workbook saved to S3 and/or local disk cache`
                );
                res
                    .status(201)
...
function saveWorkbook({ workbook, config, localCache }) {
    ...
    return Observable.from(localCache.put(workbookURL, serialized))
        ...
        .switchMap(localWorkbookPath => {
            logger.info(
                { workbook: workbook.id },
                `Saved workbook to disk at path ${localWorkbookPath}`
            );
            logger.info(
                { workbook: workbook.id },
                `Uploading workbook to S3 in bucket "${config.BUCKET}" and name "${workbookURL.pathname}"`
            );
...
function downloadWorkbookRequestHandler(req, res, localCache, config) {
    ...
    if (!req.params.workbook) {
        logger.warn(
            { req, res },
            `Error: workbook download request did not specify a workbook ID to download`
        );
    ...
    return fetchWorkbook({ workbookId, localCache, S3, BUCKET }).subscribe(
        workbook => {
            logger.info(
                { req, res, workbook: workbookId },
                `Fetched workbook successfully. Responding to download request with its contents.`
            );
            res.status(200).json(workbook);
        },
        err => {
            logger.warn(
                { req, res, err, workbook: workbookId },
                `Error fetching requested workbook for download request. Sending 404 to client.`
            );
            res.status(404).json({
                success: false,
...
function fetchWorkbook({ workbookId, localCache, S3, BUCKET }) {
    ...
    return Observable.from(localCache.get(workbookURL))
        .do(() =>
            logger.debug(
                { workbook: workbookId },
                `Workbook found in local storage cache. Will fetch local copy and skip S3.`
            )
        )
        .map(workbookFileBuffer => workbookFileBuffer.toString())
        .catch(err => {
            logger.debug(
                { err, workbook: workbookId },
                `Could not fetch workbook from local storage cache. Will try to fetch from S3 bucket ${BUCKET}.`
            );
            ...
            return Observable.from(
                ...
                logger.debug(
                    { workbook: workbookId },
                    `Fetched workbook from S3. Will cache workbook locally for future use.`
                );
                ...
                return Observable.from(localCache.put(workbookURL, s3WorkbookContent))
                    .catch(cacheErr => {
                        logger.warn(
                            { err: cacheErr, workbook: workbookId },
                            `Error trying to cache workbook from S3 locally.`
                        );
