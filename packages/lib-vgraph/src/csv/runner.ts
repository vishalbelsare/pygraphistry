...
export interface ShmBuffer extends Buffer { key: number; }
...
function forkWorkers(workers, opts) {
    return Array.from(
        { length: workers },
        (x, i) => fork(workerPath, ['' + i], { execArgv: [], ...opts })
    );
}

class ShmWriter extends Writer {
    static create() { return new ShmWriter(); }
    static alloc(size: number) {
        return size < 1
            ? Writer.alloc(size)
            : shm.create(size, 'Uint8Array', null);
    }
}
...
function writeCSVHeader(table) {
    return function writeHeader(csvFd: fs.WriteStream): Observable<fs.WriteStream> {
        return Observable
            .bindNodeCallback(csvFd.write, () => (csvFd))
            .bind(csvFd)([
                ...(table.bool_vectors || []),
                ...(table.float_vectors || []),
                ...(table.int32_vectors || []),
...
function convertAndWriteCSVRows(table, numWorkers, rowsBatchCount) {
    return function convertAndWriteRows(csvFd: fs.WriteStream) {
        return Observable
            .of({ start: 0, total: table.length })
            .expand(({ start, total }) => (start >= total)
                ? Observable.empty()
                : Observable.forkJoin(
        ...
        function convertSlice(start, total, worker, index) {
            ...
            if (idx2 - idx1 <= 0) {
                ...
                convertObs = Observable.if(
                    ...
                    Observable.of(0),
                    Observable
                        .bindNodeCallback(worker.send)
                        .bind(worker)({type: 'data', key})
