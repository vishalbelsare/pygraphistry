		...
		pivot.searchAndShape({
			...
			.subscribe(({pivot, ...rest}) => {
					...
					assert.deepEqual(pivot.results.graph, 
						[{x:1, destination: 1, 'source': 'x:0', edgeType: 'EventID->x'},
						 {x:3, destination: 3, 'source': 'x:1', edgeType: 'EventID->x'}]);
	...
	it('fromData', (done) => {
		...
		pivot.searchAndShape({
			...
			.subscribe(({pivot, ...rest}) => {
					...
					assert.deepEqual(pivot.results.graph, 
						[{x: "5", destination: "5", 'source': 'x:0', edgeType: 'EventID->x'},
						 {x: "10", destination: "10", 'source': 'x:1', edgeType: 'EventID->x'}]);
					assert.deepEqual(pivot.results.labels,
	...
	it('userEventID', (done) => {
		...
		pivot.searchAndShape({
			...
			.subscribe(({pivot, ...rest}) => {
					...
					assert.deepEqual(pivot.results.graph, 
						[{x: "5", destination: "5", 'source': 'aa', edgeType: 'EventID->x'},
						 {x: "10", destination: "10", 'source': 'bb', edgeType: 'EventID->x'}]);
					assert.deepEqual(pivot.results.labels,
	...
	it('search multiple events', (done) => {
		...
		pivot.searchAndShape({
			...
			.subscribe(({pivot, ...rest}) => {
					...
					assert.deepEqual(pivot.results.graph, 
						[{x: "a", destination: "a", 'source': 'x:0', edgeType: 'EventID->x'},
						 {x: "b", destination: "b", 'source': 'x:1', edgeType: 'EventID->x'}]);
					assert.deepEqual(pivot.results.labels,
