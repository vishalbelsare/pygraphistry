...
describe('shapeResults', function() {
    ...
    it('hypergraph does not create nulls', function(done) {
        const pivot = {
            events: [{ EventID: 'xx', y: 'z', a: undefined, b: null, c: '', d: '""' }]
        };
    ...
    it('hypergraph handles multiple non-overlapping events', function(done) {
        const pivot = {
            events: [{ EventID: 'xx', y: 'z' }, { EventID: 'yy', y: 'r', a: 1 }]
        };
        const expected = {
            graph: [
                {
    ...
    it('hypergraph handles multiple overlapping events', function(done) {
        const pivot = {
            events: [{ EventID: 'xx', y: 'z' }, { EventID: 'yy', y: 'z', a: 1 }]
        };
        const expected = {
            graph: [
                {
    ...
    it('graph only edges', done => {
        const pivot = {
            graph: { nodes: [], edges: [{ source: 'x', destination: 'y', edge: 'myedge' }] }
        };
    ...
    it('graph synthesizes edge ids', done => {
        const pivot = {
            ...
            graph: {
                nodes: [],
                edges: [{ source: 'x', destination: 'y' }, { source: 'x', destination: 'z' }]
            }
        };
        const expected = {
            labels: [],
    ...
    describe('refTypes', () => {
            ...
            const expected = {
                ...
                labels: [
                    { ...pivot.events[0], node: 'xx', type: 'EventID' },
                    {
                        ...
                        type: 'refA_colA',
                        refTypes: ['A', 'X'],
                        cols: ['refA_colA', 'refA_colB', 'refX_colX']
                    },
        ...
        it('makes shared refType entities into a strongly connected component', done => {
            const pivot = {
                events: [
                    { EventID: 'xx', refA_colA: 'a', refA_colB: 'b', refA_colC: 'c', noref: 'd' }
                ],
                template
            };
            const expected = {
