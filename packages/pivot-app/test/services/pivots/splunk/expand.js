	...
	it('handles one pivot with multiple events', () => {
		...
		assert.deepEqual(
			expand({
				...
				pivotIds: [pivot], 
				fields: [field],
				pivotCache: {[pivot]: { results: {labels: [ {node: val1, type: field}, {node: val2, type: field} ]} }}
			}),
	...
	it('handles multiple fields', () => {
		...
		assert.deepEqual(
			expand({
				...
				pivotCache: {[pivot]: { results: {labels: 
					[ {type: field1, node: val1}, {type: field2, node: val2} ]} }}
			}),
	...
	it('* skips reserved fields', () => {
		...
		assert.deepEqual(
			expand({
				...
				pivotIds: [pivot], 
				pivotCache: {[pivot]: { results: {labels: [ 
					{ node: val, type: field, ok: 1,
					  pointColor: 1, pointSize: 10, pointIcon: 'alarm', pointTitle: 'x'
				} ]} }}
			}),
	...
	it('can do colMatch=false,', () => {
		...
		assert.deepEqual(
			expand({
				...
				fields: [field],
				pivotCache: {[pivot]: { results: {labels: 
					[ {type: field, node: val}, {type: field, node: val} ]} }}
			}),
