...
export function authenticateMiddleware(convict) {
    if (!convict.get('authentication.passwordHash')) {
        log.warn(
            `Authentication disabled because "authentication.passwordHash" is not set in your config. Anybody will be able to access this service without restriction.`
        );
...
function checkLoginCredentialsMiddleware(convict) {
    return function checkLoginCredentialsMiddlewareHandler(
        providedUsername,
        providedPassword,
        authResultsCb
    ) {
        ...
        compare(providedPassword, authorizedPassword, (bcryptErr, isPasswordEqualToHash) => {
            if (bcryptErr) {
                log.error(
                    { err: bcryptErr },
                    `Error while comparing plaintext password (in authentication request for username "${providedUsername}") to the configured hashed password (in config entry 'authentication.passwordHash')`
                );
            ...
            if (providedUsername !== authorizedUsername) {
                ...
                log.warn(
                    `Rejecting authentication attempt: invalid username "${providedUsername}"`
                );
                return authResultsCb(null, false);
            } else if (!isPasswordEqualToHash) {
                // invalid password; reject authentication attempt
                log.warn(
                    `Rejecting authentication attempt: invalid password for username "${providedUsername}"`
                );
                return authResultsCb(null, false);
            } else {
                ...
                log.debug(
                    `Successfully authenticated credentials for username "${providedUsername}"`
                );
