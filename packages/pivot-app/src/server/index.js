...
app.use(
  '/icons',
  express.static(path.join(process.cwd(), './www/public/icons')),
  requestErrorHandler
);

const renderPageMiddleware = configureRenderMiddleware(
  convict,
  configureFalcorModelFactory(req => getDataSource(req, { streaming: false }))
);
