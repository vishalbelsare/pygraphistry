...
if (context) {
  if (!module.hot) {
    throw new Error(
      '[WUS] Hot Module Replacement is disabled, or webpack-universal-server is in webpack externals.'
    );
  }
  context.hotCheck = function() {
    ...
    function checkAndApply(resolve, reject) {
      return module.hot.check(false).then(updated => {
        if (updated && module.hot.status() === 'ready') {
          return module.hot
            .apply({ ignoreErrored: true })
