    ...
    return (
        <ButtonToolbar>
            <OverlayTrigger
                ...
                overlay={tooltipDeleteStep}>
                <Button
                    bsSize="xsmall"
                    disabled={index === 0 && numRows === 1}
            ...
            <ButtonGroup bsSize="xsmall" className={styles['run-insert-button-group']}>
                <OverlayTrigger
                    ...
                    placement="top"
                    delayShow={tooltipDelayShow}
                    overlay={tooltipRunStep}>
                    <Button
                        disabled={status.searching}
                        onClick={() => searchPivot({ index, pivotId, investigationId })}>
                        <Glyphicon
                            className="fa-fw"
                            glyph={status.searching ? 'hourglass' : 'play'}
                        />
                    </Button>
                </OverlayTrigger>
                <OverlayTrigger
                    ...
                    <OverlayTrigger
                        ...
                        overlay={
                            <Popover
                                ...
                                <pre className={styles.message}>{status.message}</pre>
                            </Popover>
                        }>
