...
const createPivotCellOnChangeContainer = changeHandler =>
    ...
    ({ paramKey, previousPivots }, setPivotAttributes) => event =>
        ...
        setPivotAttributes({
            [`pivotParameters.${paramKey}`]:
                event.target.value === allPivotsIdValue
...
export default function PivotCell({ paramUI, ...props }) {
    ...
    return shouldNotRender ? null : <PivotCellComponent paramUI={paramUI} {...props} />;
}

function Label({ id, paramUI, paramKey }) {
    return (
        <FormGroup
            className={styles['pivot-label-param']}
...
function TextCell({ id, paramKey, paramValue, paramUI, onChange }) {
    return (
        <FormGroup
            className={styles['pivot-text-param']}
...
function TextareaCell({ id, paramKey, paramValue, paramUI, onChange }) {
    return (
        <FormGroup
            className={styles['pivot-textarea-param']}
...
function PivotCombo({ id, paramKey, paramValue, paramUI, previousPivots, onChange }) {
    ...
    options = (previousPivots || []).map(({ id, pivotTemplate }, index) => ({
        value: id,
        label: `Pivot ${index + 1} (${pivotTemplate.name})`
    }));
...
function ComboCell({ id, paramKey, paramValue, paramUI, onChange }) {
    ...
    return (
        <FormGroup
            className={styles['pivot-select-param']}
...
function BoolCell({ id, paramKey, paramValue, paramUI, onChange }) {
    return (
        <FormGroup
            className={styles['pivot-bool-param']}
...
function MultiCell({ id, paramKey, paramValue, paramUI, onChange }) {
    ...
    const optionIds = options.reduce(
        (ids, { id }) => ({
            ...ids,
            [id]: true
        }),
    ...
    const multiOptions = options.concat(
        (paramValue || []).filter(id => !optionIds.hasOwnProperty(id)).map(id => ({ id, name: id }))
    );
