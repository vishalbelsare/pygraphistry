...
export default withSchema((QL, { get, set }, services) => {
  ...
  return QL`{
        ['id', 'length', 'resultCount', 'resultSummary']: ${readOnlyHandler},
        ['status', 'enabled', 'description', 'pivotTemplate']: ${readWriteHandler},
        pivotParameters: {
            [{ keys }]: ${readWriteHandler}
        },
        searchPivot: ${searchPivotHandler}
    }`;
});
...
function captureErrorAndNotifyClient(pivotIds) {
  ...
  return function(e) {
    if (e instanceof ErrorObservable) {
      return e.catch(e => Observable.of(e)).switchMap(e => captureErrorAndNotifyClientFlat(e));
    } else {
