...
class HttpConnector extends Connector {
  ...
  search(url, { method = 'GET', timeout, body, headers = {} } = {}) {
    ...
    const req = method === 'GET' ? get(reqBase) : post({ ...reqBase, body });
      ...
      .do(([response]) => {
        ...
        if (statusCode === 401) {
          ...
          throw new VError(
            ...
            'URL gave an unexpected response code: ' + (statusCode || 'none available'),
            info
          );
        }
      })
      .timeoutWith(
        (timeout || this.timeout_s) * 1000,
        Observable.throw(
          new VError(
            {
              name: 'Timeout',
  ...
  healthCheck() {
    return Observable.of('Health checks passed').do(() =>
      log.info('Health checks passed for HTTP connnector')
    );
  }
}
