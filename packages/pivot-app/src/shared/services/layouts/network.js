...
function mergeLabels(x, y) { return x | y; }
function mergeSrcDstLabels(src, dst) { return src | (dst << 2); }
function isUnlabeled(x) { return (x | 0) === 0; }
        ...
        [src,dst,all].
            forEach((group = []) => {
                group.forEach(({node}) => {
                    ...
                    all_io[node] = isUnlabeled(entityLabelRaw) ? mergeLabels(all_io[node], label) 
                        : entityLabelRaw;
                });
            });
