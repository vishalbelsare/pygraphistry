...
export class SimpleFileSystemStore {
  constructor({ loadApp, pathPrefix, entityName, createModel, serializeModel, cache }) {
    ...
    this.cacheService = new SimpleCacheService({
      ...
      loadById: entityId =>
        ...
        (this.entities[entityId] = globAsObservable(path.resolve(pathPrefix, `${entityId}.json`))
          .mergeAll()
          .take(1)
          .single()
  ...
  loadById(ids) {
    return this.cacheService.loadByIds(ids).do(res => {
  ...
  unloadById(ids) {
    return this.cacheService.unloadByIds(ids).do(res => {
      const entity = res[this.entityName];
    ...
    return this.loadById(ids)
      .mergeMap(res => {
        ...
        return renameAsObservable(filePath, `${filePath}.deleted`)
          .catch(e => (e.code === 'ENOENT' ? Observable.of(null) : Observable.throw(e)))
          .map(() => res);
      })
      .do(res => {
        const entity = res[this.entityName];
