...
export function createInvestigation({
  ...
  return loadUsersById({ userIds })
    ...
    .do(({ newInvestigation }) => log.debug(`Created new investigation ${newInvestigation.id}`));
}
  ...
  return loadInvestigationsById({ investigationIds })
    .mergeMap(({ app, investigation }) => {
      ...
      return persistPivotsById({ pivotIds })
        .toArray()
        .switchMap(() => unlinkPivotsById({ pivotIds: investigation.detachedPivots }).toArray())
        .switchMap(() => persistInvestigationsById({ investigationIds: [investigation.id] }))
...
export function removeInvestigationsById({
  ...
  return loadUsersById({ userIds: userIds }).mergeMap(({ user }) => {
    ...
    if (investigationIds.includes(activeInvestigationId)) {
      user.activeInvestigation = newInvestigations.length > 0 ? user.investigations[0] : undefined;
    }
