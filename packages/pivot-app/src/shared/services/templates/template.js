...
export const PARAM_OVERRIDE_WHITELIST = [
    'placeholder',
    'options',
    'isVisible',
    'label',
...
export class PivotTemplate {
    constructor({ id, name, tags = [], parameters = [] }) {
        if (!id || !name) {
            throw new Error(
                `Pivot template expects fields 'id' and 'name', got '${id}' and '${name}'`
            );
        }
    ...
    clone(settings = {}) {
        ...
        const newParameters = (settings.parameters || []).filter(
            settingParam =>
                0 ===
                this.parameters.filter(templateParam => templateParam.name === settingParam.name)
                    .length
        );
    ...
    cloneParameters(template, settings, newParameters) {
        if ('parameters' in settings) {
            const newParametersDict = newParameters.reduce((acc, param) => {
    ...
    addTemplateNamespace(templateId, parameters) {
        return parameters
            .map(parameter => ({
                id: this.tagTemplateNamespace(templateId, parameter.name),
                ...parameter
            }))
