...
import { createPivotModel, serializePivotModel } from '../models';
...
export function pivotStore(loadApp, pathPrefix, loadTemplatesById, pivotsByIdCache = {}) {
    ...
    return {
        loadPivotsById: ({ pivotIds }) => {
            return store
                .loadById(pivotIds) //Fill in default pivotParameters from template
                .mergeMap(
                    ({ pivot }) =>
                        loadTemplatesById({
                ...
                .map(({ pivot, template }) => {
                    ...
                    for (const fld in template.pivotParametersUI.value) {
                        if (!(fld in pivot.pivot.pivotParameters)) {
                            pivot.pivot.pivotParameters[fld] =
                                template.pivotParametersUI.value[fld].defaultValue;
                        }
                    }
                    return pivot;
                });
        },
        unloadPivotsById: ({ pivotIds }) => store.unloadById(pivotIds),
        persistPivotsById: ({ pivotIds }) => store.persistById(pivotIds),
        unlinkPivotsById: ({ pivotIds }) => store.unlinkById(pivotIds)
    };
}
