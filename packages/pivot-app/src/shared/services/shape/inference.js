...
function inferenceEdges (edges) {
    ...
    const hyperedges = 
        edges
            .filter((edge) => edge.refType && edge.edgeType.indexOf('EventID->') === 0);

    //[ [edge] ]
    const hyperedgesGroupedBySourceRef = 
        Object.values(
            _.groupBy(
                hyperedges,
                (edge) => JSON.stringify([edge.source, edge.refType])));

    //[ [edge] ]
    // groups of at least 2 uniq entities
    const equalityClasses = 
        hyperedgesGroupedBySourceRef
            .map((edges) => _.uniq(edges, 'destination'))
            .filter((edges) => edges.length > 1);
 
    //[ [ [edge] ] ]
    // create edges from cross-product (diagnonal)
    const newEdgesByClass =
        equalityClasses.map((edges) =>    
            edges.map((edgeA, i) => 
                edges
                    .filter((edgeB, j) => j > i)
                    .map((edgeB) => ({
...
export function dedupeHyperedges(edges) {
    ...
    edges.forEach((edge) => {
        ...
        if (existingCanonicalEdge) { //merge                    
            existingCanonicalEdge.cols.push(edge.col); 
        } else { //swap hyperedge singleton with a hyperedge group        
