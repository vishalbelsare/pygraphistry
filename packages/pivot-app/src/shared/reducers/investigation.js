...
function graphInvestigation(action$) {
    return action$
        ...
        .mergeMap(actionsById =>
            actionsById.switchMap(({ investigationId, falcor, length }) => {
                return falcor
                    .set($value(['status'], { searching: true, ok: true }))
                    .concat(
                        Observable.range(0, length).concatMap(index =>
                            Observable.merge(
                                falcor.set(
                                    $value(`pivots[${index}].status`, { searching: true, ok: true })
                                ),
...
function dismissAlert(action$) {
    return action$
        ...
        .groupBy(({ id }) => id)
        .mergeMap(actionsById =>
            actionsById.switchMap(({ falcor }) => {
                return falcor.set($value(`['status']`, { ok: true })).progressively();
            })
        )
        .ignoreElements();
}
...
function searchPivot(action$) {
    return action$
        ...
        .groupBy(({ investigationId }) => investigationId)
        .mergeMap(actionsById =>
            actionsById.switchMap(({ investigationId, pivotId, falcor }) => {
                ...
                return falcor
                    ...
                    .concat(
                        topLevelModel.call(`pivotsById['${pivotId}'].searchPivot`, [
                            investigationId
                        ])
                    )
                    .concat(
...
function splicePivot(action$) {
    return action$
        ...
        .groupBy(({ id }) => id)
        .mergeMap(actionsById =>
            actionsById.switchMap(({ falcor, index }) => {
                return falcor.call(`splicePivot`, [index]).progressively();
            })
        )
        .ignoreElements();
}
...
function insertPivot(action$) {
    return action$
        ...
        .mergeMap(actionsById =>
            actionsById.switchMap(({ falcor, index }) => {
                return falcor.call(`insertPivot`, [index]).progressively();
            })
        )
        .ignoreElements();
}
