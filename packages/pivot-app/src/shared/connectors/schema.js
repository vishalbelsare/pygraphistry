...
export default withSchema((QL, { get, set }, services) => {
  ...
  return QL`{
        ['id', 'name', 'status', 'lastUpdated']: ${readWriteHandler},
...
function checkStatusCallRoute({ loadConnectorsById, checkConnector }) {
  return function({ connectorIds }) {
    ...
    const statusReports = Observable.from(connectorIds).map(connectorId =>
      $pathValue(`connectorsById['${connectorId}'].status`, {
        enabled: false,
        level: 'default',
