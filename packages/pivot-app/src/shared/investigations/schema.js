...
export default withSchema((QL, { get, set }, services) => {
    ...
    return QL`{
        ...
        ['id', 'name', 'tags', 'url', 'datasetName', 'datasetType', 'controls', 'status', 'description', 'time', 'createdOn', 'modifiedOn', 'layout', 'axes', 'edgeOpacity']: ${readWriteHandler},
        pivots: {
        ...
        return splicePivot({
            ...
            .mergeMap(({ investigation }) => {
                return [
                    ...
                    $invalidation(
                        `investigationsById['${investigationIds}']['pivots'][${0}..${investigation
                            .pivots.length}]`
                    )
                ];
            })
    ...
    return function(path, args) {
        ...
        return insertPivot({ loadInvestigationsById, investigationIds, pivotIndex })
            .mergeMap(({ investigation, insertedIndex }) => {
                ...
                const values = [
                    $pathValue(
                        `investigationsById['${investigation.id}']['pivots'].length`,
                        length
                    ),
                    $pathValue(
                ...
                if (insertedIndex < length - 1) {
                    // Inserted pivot is not the last one in the list
                    values.push(
                        $invalidation(
                            `investigationsById['${investigation.id}']['pivots'][${insertedIndex +
                                1}..${length - 1}]`
        ...
        return uploadGraph({
            ...
            .mergeMap(({ investigation }) => {
                ...
                return [
                    ...
                    $pathValue(
                        `investigationsById['${investigationIds}'].axes`,
                        investigation.axes
                    ),
                    $pathValue(
                        `investigationsById['${investigationIds}'].edgeOpacity`,
                        edgeOpacity
                    ),
                    $pathValue(
                        `investigationsById['${investigationIds}'].status`,
                        investigation.status
                    ),
                    $pathValue(
                        `investigationsById['${investigationIds}'].controls`,
                        investigation.controls
                    ),
                    $pathValue(
                        `investigationsById['${investigationIds}'].datasetName`,
                        investigation.datasetName
                    ),
                    $pathValue(
                        `investigationsById['${investigationIds}'].datasetType`,
                        investigation.datasetType
                    ),
                    $pathValue(
                        `investigationsById['${investigationIds}'].eventTable`,
...
function saveCallRoute({
    ...
    return function(path) {
        ...
        return saveInvestigationsById({
            ...
            .mergeMap(({ investigation }) => [
                $pathValue(
                    `investigationsById['${investigationIds}'].modifiedOn`,
                    investigation.modifiedOn
                ),
