...
const HandleTagUpdates = mapPropsStream((propsStream) => {
    ...
    const handleAddition = (tag) => push({
        type: 'push', tag
    });
    const handleDelete = (index) => splice({
        type: 'splice', index
    });
    const handleDrag = (tag, index, newIndex) => drag({
        type: 'drag', tag, index, newIndex
    });
...
function InvestigationTags({
    ...
    handleDrag, handleDelete, handleAddition
}) {
    return (
        <ReactTags tags={tags}
                   autofocus={autoFocus}
