...
loadVGraph(metadata)
    ...
    .do(({ table, csvFilePath }) =>
        printTime(false, `converted and wrote ${table.name} table at ${csvFilePath}`)
    )
    .concatMap(
        ({ client, table, csvFileName, csvFilePath }) =>
            createTable({ client, table, csvFileName }),
    ...
    .subscribe({
        error(err) {
            debugger;
            printTime(true, `got err`, err);
        },
        complete() {
            debugger;
            printTime(false, 'done');
        }
    });
...
function timer() {
    ...
    return function print(err, ...args) {
        ...
        (err ? console.error : console.log).apply(console, [
            `%d.%ds:`,
            hrend[0],
            (hrend[1] / 1000000) | 0,
            ...args
