...
export function loadVGraph(options: LoaderOptions) {
    return Observable.of({ ...options, loaded: false })
        .expand(
            ({ type, loaded, ...opts }) =>
                loaded === true
                    ? Observable.empty()
                    : downloadDataset(opts).map<any, LoaderResults>(vDecoders[type])
        )
        .takeLast(1);
}
...
function decodeJSONMeta({ buffer, opts }) {
    ...
    return {
        ...
        nodes: meta.nodes[0],
        edges: meta.edges[0],
        type: meta.type || 'vgraph',
        loaded: false
    };
}

function downloadDataset(opts: DownloadOptions) {
    const url = urlParse(opts.dataset),
        { protocol } = url;
...
function loadLastModifiedS3(transport, url): Observable<number> {
    return Observable.bindNodeCallback(transport.headObject).bind(transport)(url).pluck(
        'LastModified'
    );
}

function loadLastModifiedWWW(transport, url): Observable<number> {
    return Observable.bindCallback(transport.request).bind(transport)({
        ...url,
        method: 'HEAD'
...
function loadDatasetS3(transport, url) {
    return Observable.bindNodeCallback(transport.getObject).bind(transport)(url).pluck('Body');
}

function loadDatasetWWW(transport, url) {
    return Observable.bindCallback(transport.get).bind(transport)(url)
        .do(res => res.setEncoding('binary'))
        .flatMap(res =>
            Observable.fromEvent(res, 'data').takeUntil(Observable.fromEvent(res, 'end'))
        )
