...
export function createDB(opts: CreateDBOptions) {
    ...
    const startMapDCmd = runCmd(
        ...
        [`--no-frontend`, `--non-interactive`, `--data`, mapdDataDir],
        'startmapd',
        { cwd: mapdDir, detached: true, ignoreErrors: true }
    );
    const mapdQLCmd = runCmd(
        path.join(mapdDir, './bin/mapdql'),
        [`-u`, `mapd`, `-p`, `HyperInteractive`],
        'mapdql',
        {
            cwd: mapdDir,
            stdin: Observable.of(
    ...
    return startMapDCmd
        ...
        .flatMapTo(mapdQLCmd.do(x => console.log(`mapdql msg: ${x}`)).takeLast(1))
        .filter((x, index) => index === 0)
