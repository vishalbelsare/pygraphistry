...
function createColumnVectors(nodeVecs: VectorMetadata[], edgeVecs: VectorMetadata[]) {
    ...
    const columnDescs = [
        ...
        { column_type: 'uint32', table_name: 'edges', column_name: EDGE_ID, public_name: 'edge' },
        {
            column_type: 'uint32',
            table_name: 'edges',
            column_name: EDGE_SRC,
            public_name: 'source'
        },
        {
            column_type: 'uint32',
            table_name: 'edges',
            column_name: EDGE_DST,
            public_name: 'destination'
        },
...
function assignColumnMappings(vecs: VectorMetadata[], attrs: AttributesMetadata) {
    ...
    return vecs.map(vector => {
        return !vector.attributeVectorName
            ? vector
            : mapVectorTypes(vector, attrNamesToDefaultTypes[vector.attributeVectorName]);
    }) as VectorMetadata[];
...
function readVGraph(buffer: Buffer | Uint8Array) {
    ...
    return new Pbf(buffer).readFields(
        (tag, vectors, pbf) => {
            let vecs;
...
function readVector(pbf: any, attributeVectorName: string) {
    ...
    return [{ name, target, attributeVectorName, bytes }];
}
...
function readEdges(pbf: any, edgeCount: number) {
    ...
    do {
        ...
        do {
            ...
            switch (val >> 3) {
                case 1:
                    srcs[edgeIndex] = pbf.readVarint();
                    break;
                case 2:
                    dsts[edgeIndex] = pbf.readVarint();
                    break;
                default:
                    pbf.skip(val);
            }
