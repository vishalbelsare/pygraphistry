...
VectorGraph._readField = function(tag, obj, pbf) {
    if (tag === 1) obj.version = pbf.readVarint();
        ...
        obj.int32_vectors.push(
            VectorGraph.Int32AttributeVector.read(pbf, pbf.readVarint() + pbf.pos)
        );
    else if (tag === 11)
        obj.int64_vectors.push(
            VectorGraph.Int64AttributeVector.read(pbf, pbf.readVarint() + pbf.pos)
        );
    else if (tag === 12)
        obj.float_vectors.push(
            VectorGraph.FloatAttributeVector.read(pbf, pbf.readVarint() + pbf.pos)
        );
    else if (tag === 13)
        obj.bool_vectors.push(
            VectorGraph.BoolAttributeVector.read(pbf, pbf.readVarint() + pbf.pos)
        );
...
VectorGraph.Bindings._readField = function(tag, obj, pbf) {
    if (tag === 1) obj.idField = pbf.readString();
        ...
        obj.mappings.push(VectorGraph.Mapping.read(pbf, pbf.readVarint() + pbf.pos));
};
