...
const [doesNameIndicateTimeColumn, doesNameIndicateDateColumn] = [/time/i, /date/i].map(r =>
    r.test.bind(r)
);
...
function convertNumber(format?: string) {
    return format === 'unix' ? convertUnix : !format ? convertAny : convertWithFormat(format);
}

function convertString(format?: string) {
    return !format ? convertAny : convertWithFormat(format);
}
